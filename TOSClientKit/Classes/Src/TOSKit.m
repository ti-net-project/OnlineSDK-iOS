...
- (void)onReceived:(TOSMessage *)message left:(int)nLeft{
    ...
    /// 机器人订单事件  triggerQuickAccess： 快捷入口。triggerToolBar：更多面板
    else if ([message.type isEqualToString:@"ordertrigger"]) {
        if (self->reckitMessageDelegate && [self->reckitMessageDelegate respondsToSelector:@selector(onTIMEvent:withMessage:)]) {
            TIMOrderEventMessage * eventMessage = (TIMOrderEventMessage *)message.content;
            [self->reckitMessageDelegate onTIMEvent:eventMessage.event withMessage:message];
        }
    }
...
-(TIMMessageFrame *)analysisHistoryWithModel:(OnlineChatRecordModel*)model withItemReload:(BOOL)reload {
    ...
    if ([model.messageType intValue] > 32 && [model.messageType intValue] != 38) {
