...
//        }];
        ...
        if ([TOSKitCustomInfo shareCustomInfo].chatBox_moreButton_enable) {
            [self.moreButton mas_TIMmakeTIMConstraints:^(TIMMASConstraintMaker *make) {
                ...
                make.right.mas_TIMequalTo(-[TOSKitCustomInfo shareCustomInfo].chatBox_itemRightPadding).priorityHigh();
                make.bottom.mas_TIMequalTo(-[TOSKitCustomInfo shareCustomInfo].chatBox_itemBottomSpacing);
                make.width.mas_TIMequalTo([TOSKitCustomInfo shareCustomInfo].chatBox_Item_Width).priorityHigh();
                make.height.mas_TIMequalTo([TOSKitCustomInfo shareCustomInfo].chatBox_Item_Width);
            }];
        }
...
- (void)setupStaffCommentTotalCount:(NSNumber *)staffCommentTotalCount {
    ...
    for (NSInteger i = 0; i < self.barDataCount.count; i++) {
        ...
        BOOL addItem = YES;
        if (self.barDataCount[i].type == TOSQuickEntryEventTypeTicketPlugin) {
...
- (UIButton *) moreButton
{
    if (_moreButton == nil) {
        ...
        _moreButton.contentMode = UIViewContentModeScaleAspectFill;
...
- (void)ICChatBarFunctionViewDidSelect:(NSInteger)index withItem:(nonnull ICChatBarItemView *)item {
    
    
    if (self.barDelegate && [self.barDelegate respondsToSelector:@selector(ICChatBoxBarWithItemDidselectedIndex:withModel:)]) {
        [self.barDelegate ICChatBoxBarWithItemDidselectedIndex:index withModel:item.model];
    }
...
/// - Parameters:
///   - update: YES: 添加，NO：移除
///   - index: 0: 语音，1: 表情，2: 更多
- (void)updateFunctionBtn:(BOOL)update withIndex:(NSInteger)index {
...
//                        NSLog(@"更多按钮不存在，新建高宽度和右侧间距");
                        ...
                        [self.moreButton mas_TIMmakeTIMConstraints:^(TIMMASConstraintMaker *make) {
                            ...
                            make.right.mas_TIMequalTo(-[TOSKitCustomInfo shareCustomInfo].chatBox_itemRightPadding).priorityHigh();
                            make.bottom.mas_TIMequalTo(-[TOSKitCustomInfo shareCustomInfo].chatBox_itemBottomSpacing);
//                            NSLog(@"添加后。当前按钮的大小和位置：%@", NSStringFromCGRect(self.moreButton.frame));
                        }];
...
//                        NSLog(@"更多按钮存在，更改宽度和右侧间距： %@", NSStringFromCGRect(self.moreButton.frame));
                        [self.moreButton mas_TIMupdateTIMConstraints:^(TIMMASConstraintMaker *make) {
                            make.width.mas_TIMequalTo([TOSKitCustomInfo shareCustomInfo].chatBox_Item_Width).priorityHigh(); /// 确保宽度和高度
                            make.height.mas_TIMequalTo([TOSKitCustomInfo shareCustomInfo].chatBox_Item_Width);
                            make.right.mas_TIMequalTo(-[TOSKitCustomInfo shareCustomInfo].chatBox_itemRightPadding).priorityHigh(); /// 距父视图右侧
                            make.bottom.mas_TIMequalTo(-[TOSKitCustomInfo shareCustomInfo].chatBox_itemBottomSpacing); /// 底部对齐
//                            NSLog(@"当前按钮的大小和位置：%@", NSStringFromCGRect(self.moreButton.frame));
                        }];
                        
                    }
                    
                    if (self.faceButton.tos_width>0) {
                        [self.faceButton mas_TIMupdateTIMConstraints:^(TIMMASConstraintMaker *make) {
                            make.right.equalTo(self.mas_TIMright).offset(-([TOSKitCustomInfo shareCustomInfo].chatBox_itemRightPadding+TOSKitCustomInfo.shareCustomInfo.chatBox_Item_Width+TOSKitCustomInfo.shareCustomInfo.chatBox_itemSpacing));
                        }];
                    } else {
                        /// 如果没有有表情按钮，输入框靠近更多按钮的左侧距离更新
                        [self.textView mas_TIMupdateTIMConstraints:^(TIMMASConstraintMaker *make) {
                            make.right.equalTo(self.moreButton.mas_TIMleft).offset(-[TOSKitCustomInfo shareCustomInfo].chatBox_itemSpacing);
                        }];
                        [self.talkButton mas_TIMupdateTIMConstraints:^(TIMMASConstraintMaker *make) {
                            make.right.equalTo(self.moreButton.mas_TIMleft).offset(-[TOSKitCustomInfo shareCustomInfo].chatBox_itemSpacing);
                        }];
...
//                    NSLog(@"本地没有开启更多按钮，但是后端配置中有更多功能");
                    ...
                    if (self.faceButton.tos_width>0) {
//                        [self.faceButton mas_TIMremakeTIMConstraints:^(TIMMASConstraintMaker *make) {
//                            make.right.equalTo(self.mas_TIMright).offset(-TOSKitCustomInfo.shareCustomInfo.chatBox_itemRightPadding);
//                        }];
...
//                    self.moreButton.hidden = YES;
    ...
    [UIView animateWithDuration:0.3 animations:^{
        [self layoutIfNeeded];
//        NSLog(@"当前输入区域的大小：%@", NSStringFromCGRect(self.frame));
//        NSLog(@"最终处理后 当前按钮的大小和位置：%@", NSStringFromCGRect(self.moreButton.frame));
    }];
