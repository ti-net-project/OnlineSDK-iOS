...
//            make.centerY.equalTo(self.scrollView.mas_TIMcenterY);
        ...
        [item.warnLabel mas_TIMmakeTIMConstraints:^(TIMMASConstraintMaker *make) {
            ...
            make.bottom.equalTo(item.contentLabel.mas_TIMbottom).offset(-23.f);
...
- (void)tapTouch:(UIGestureRecognizer *)gesture {
    ICChatBarItemView * item = (ICChatBarItemView *)gesture.view;
    item.userInteractionEnabled = NO;
    if (self.delegate && [self.delegate respondsToSelector:@selector(ICChatBarFunctionViewDidSelect:withItem:)]) {
        [self.delegate ICChatBarFunctionViewDidSelect:gesture.view.tag withItem:item];
    }
    /// 延迟 0.5 秒后恢复交互
