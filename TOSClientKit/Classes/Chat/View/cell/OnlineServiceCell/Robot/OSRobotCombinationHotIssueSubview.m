...
//        self.title.text = @"猜你想问";
    ...
    if ([model.type isEqualToString:@"31"]) {
        ...
        if (model.cards &&
            ...
            CombinationDataModel *data;
            
            if ([model.type isEqualToString:@"34"]) {
                __block BOOL isBlankName = NO;
                [model.data[model.specialSubjectSelect?:0].tabList enumerateObjectsUsingBlock:^(CombinationDataModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    [array addObject:[NSString stringWithFormat:@"%@",obj.name?:@""]];
                    if (idx == 0) {
                ...
                if (isBlankName) {
                    self.segmentC.hidden = YES;
                    self.segmentC.selectedSegmentIndex = 0;
                    self.segmentC.frame = CGRectMake(0, self.title.tos_bottom + 12.f, model.contentF.size.width, 0.f);
                    self.subIssueView.frame = CGRectMake(0.f, self.title.tos_bottom + 12.f, model.contentF.size.width, model.tableViewH);
                } else {
                ...
                data = model.data[model.specialSubjectSelect?:0].tabList[model.selectData?:0];
            } else {
                [model.data enumerateObjectsUsingBlock:^(CombinationDataModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    [array addObject:[NSString stringWithFormat:@"%@",obj.name?:@""]];
                }];
                ...
                self.segmentC.selectedSegmentIndex = model.selectData?:0;
                self.subIssueView.frame = CGRectMake(0.f, self.segmentC.tos_bottom, model.contentF.size.width, model.tableViewH);
                data = model.data[model.selectData?:0];
            }
            
            NSArray <NSString *>*items = [self getSubIssueItem:data.topics
                                                    selectPage:data.selectPageData?:0];
            NSArray <KnowledgeDataModel *>*knowledges = [self getSubIssueItem:data.knowledge
                                                                   selectPage:data.selectPageData?:0];
...
- (void)refreshBtnAction:(UIButton *)sender {
    ...
    if (model.cards &&
        ...
        if ([model.type isEqualToString:@"34"]) {
            data = model.data[model.specialSubjectSelect?:0].tabList[model.selectData?:0];
            index = [self reloadSubIssuePage:data.topics
                                 currentPage:data.selectPageData?:0];
            ((CombinationDataModel *)(((NSMutableArray <CombinationMessage *>*)self.tempModelFrame.model.message.content)[self.indexPath.row]).data[model.specialSubjectSelect?:0].tabList[model.selectData?:0]).selectPageData = index;
        } else {
            data = model.data[model.selectData?:0];
            index = [self reloadSubIssuePage:data.topics
                                 currentPage:data.selectPageData];
