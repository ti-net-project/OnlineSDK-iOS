...
@property (nonatomic, strong) OSRobotCombinationSpecialSubjectSubview *specialSubject;
...
- (void)setModelFrame:(TIMMessageFrame *)modelFrame
{
    ...
    [self.specialSubject removeFromSuperview];
    self.specialSubject = nil;
    ...
    [elements enumerateObjectsUsingBlock:^(CombinationMessage * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([obj.type isEqualToString:@"5"] ||
            ...
            if ([obj.type isEqualToString:@"34"]) {
                OSRobotCombinationSpecialSubjectSubview *specialSubject = [[OSRobotCombinationSpecialSubjectSubview alloc] initWithFrame:modelFrame.specialSubjectF];
                [specialSubject setWithModel:obj];
                specialSubject.tempModelFrame = self.tempModelFrame;
                specialSubject.indexPath = [NSIndexPath indexPathForRow:idx inSection:0];
                self.specialSubject = specialSubject;
                [self.contentView addSubview:self.specialSubject];
            }
    ...
    [elements enumerateObjectsUsingBlock:^(CombinationMessage * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (obj.clickLikeFeature) {
            ...
            if (obj.unHelpfulText &&
                obj.unHelpfulText.length > 0) {
                self.unHelpfulTextView.text = obj.unHelpfulText;
            } else {
                self.unHelpfulTextView.text = @"";
            }
    ...
    if ([textView isEqual:self.unHelpfulTextView] &&
        ...
        [richModels enumerateObjectsUsingBlock:^(CombinationMessage * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if (obj.clickLikeFeature) {
                ...
                obj.unHelpfulText = self.unHelpfulTextView.text;
                if (![kitUtils isBlankString:obj.answerSource]) {
                    *stop = YES;    //存在来源时，提前停止
                }
            }
