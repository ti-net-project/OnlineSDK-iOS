...
- (void)setModelFrame:(TIMMessageFrame *)modelFrame
{
    ...
    self.picView.frame = modelFrame.picViewF;
    ...
    if ([TOSKitCustomInfo shareCustomInfo].chatBubble_CornerType != BubbleCornerTypeAll && [TOSKitCustomInfo shareCustomInfo].chatBubble_CornerType != BubbleCornerTypeNormal) {
        self.picView.layer.maskedCorners = self.bubbleView.layer.maskedCorners;
    }
    
  
    if (modelFrame.model.isSender) {    // 发送者
        self.picView.layer.cornerRadius = [TOSKitCustomInfo shareCustomInfo].senderBubble_cornerRadius;
        if (image == nil) {
            TIMKitLog(@"image == nil");
        }else{
            // 如果本地路径的图片存在 那么直接显示
        ...
        self.picView.layer.cornerRadius = [TOSKitCustomInfo shareCustomInfo].receiveBubble_cornerRadius;
        NSString *orgImgPath= @"";
        if ([strPicType isEqualToString:kGIFTimageType]) {
