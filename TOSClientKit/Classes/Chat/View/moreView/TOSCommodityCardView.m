...
//        self.backgroundColor = UIColor.orangeColor;
        ...
        self.sendBtn.tos_width = [self.sendBtn.titleLabel.text defaultLabelSizeWithMaxHeight:self.sendBtn.tos_height andFont:self.sendBtn.titleLabel.font].width;
        self.sendBtn.tos_left = self.bgView.tos_width - self.sendBtn.tos_width - 12.f;
        
        self.price.tos_width = self.sendBtn.tos_left - self.commodityTitle.tos_left - 10.f;
...
- (UILabel *)price {
    if (!_price) {
        _price = [[UILabel alloc] initWithFrame:(CGRectMake(self.commodityTitle.tos_left, self.descriptionsLabel.tos_bottom+2.0, 100.0, 22.0))];
...
- (UIButton *)sendBtn {
    if (!_sendBtn) {
        _sendBtn = [[UIButton alloc] initWithFrame:(CGRectMake(self.bgView.tos_width-80.0-12.0, self.descriptionsLabel.tos_bottom, 80.0, 22.0))];
...
- (UILabel *)descriptionsLabel {
    if (!_descriptionsLabel) {
        _descriptionsLabel = [[UILabel alloc] initWithFrame:(CGRectMake(self.commodityTitle.tos_left, self.commodityTitle.tos_bottom, self.commodityTitle.tos_width, 18.0f))];
