    ...
    if (model.isSender) {
        ...
        if ([model.message.type isEqualToString:TypeText] ||             // 文字
            ...
            if (![kitUtils isBlankString:model.message.content]) {
                ...
                /// 默认排队中
                NSString * leaveQueueStr = @"";
                /// 如果结束了会话或者进入人工，需要把放弃排队置空
                if ([TIMClient.sharedTIMClient getLibOnlineStatus] == TinetChatStatusTypeRobot &&
                    model.message.hideLeaveQueueBtn == NO) {
                    leaveQueueStr = @" 放弃排队";
                } else {
                    leaveQueueStr = @"";
                }
...
//            [self queueMessageFrameWithModel:model bubblePadding:bubblePadding cellMinW:cellMinW topViewH:topViewH headToBubble:headToBubble topTimeViewH:topTimeViewH arrowWidth:arrowWidth topViewToBubbleH:2+20 cellMargin:cellMargin timeShowLength:timeShowLength];
        ...
        else if ([model.message.type isEqualToString:TypeOrderCard]) {
            
            TOSOrderMessage *cardMessgae = (TOSOrderMessage *)model.message.extra;
//            NSLog(@"订单数组：%@", cardMessgae.productList);
//            NSLog(@"订单扩展数据== extraData: %@   extraInfoArr: %@", cardMessgae.extraData, cardMessgae.extraInfoArr);
//            cardMessgae.extraData = dictionary[@"extraData"];
//            cardMessgae.extraInfoArr = dictionary[@"extraInfo"];
            CGFloat chatOrderWidth = chatLabelMax;
            if (chatOrderWidth < 247.0) {
                chatOrderWidth = 247.0f;
            }
//            if ((App_Frame_Width-CGRectGetWidth(_headImageViewF)-headToBubble) <= headToBubble) {
//                chatOrderWidth = App_Frame_Width-CGRectGetWidth(_headImageViewF)-headToBubble-headToBubble;
//            }
            
            CGSize chateLabelSize = CGSizeMake(chatOrderWidth, cardMessgae.totalHeight);
            CGSize bubbleSize     = CGSizeMake(chateLabelSize.width + arrowWidth, chateLabelSize.height);
            CGSize topViewSize    = CGSizeMake(cellMinW+bubblePadding*2, topViewH);
            _bubbleViewF          = CGRectMake(CGRectGetMinX(_headImageViewF) - headToBubble - bubbleSize.width, cellMargin+topViewH+topViewToBubbleH+topTimeViewH + sendNickNameToBubbleSpacing, bubbleSize.width, bubbleSize.height);
            CGFloat x             = CGRectGetMinX(_bubbleViewF)+bubblePadding;
            _topViewF             = CGRectMake(CGRectGetMinX(_headImageViewF) - timeShowLength , cellMargin+topTimeViewH,topViewSize.width,topViewSize.height);
            ...
            /// 订单卡片的数据是由接入侧提供的元数据，敏感词label的大小直接置空
            _sensitiveWordsLabelF = CGRectZero;
        ...
        if ([model.message.type isEqualToString:TypeText] ||           // 文本
            ...
            __block CGFloat bubbleViewSpecialSubjectAppendY = 0;
                ...
                if ([obj.type isEqualToString:@"2"] ||
                    ...
                    if (contentWidth < chateLabelSize.width) {
                        contentWidth = chateLabelSize.width;
                           ...
                           [obj.type isEqualToString:@"34"]) {
                    
                    __block CGSize chateLabelSize = CGSizeMake(chatLabelMax, 0.f);
                    __block CGFloat tableViewHeight = 0.f;
                    
                    if ([obj.type isEqualToString:@"34"]) {
                        bubbleViewSpecialSubjectAppendY += 78.f;
                        bubbleViewSpecialSubjectAppendY += 8.f;
                    }
                    ...
                    if ([obj.type isEqualToString:@"31"]) {
                        ...
                        if (obj.cards &&
                            ...
                            if ([obj.type isEqualToString:@"34"]) {
                                __block CGFloat segmentHeight = 29.f;
                                __block CGFloat newSegmentHeight = 29.f;
                                __block BOOL isBlankName = NO;
                                [obj.data[obj.specialSubjectSelect?:0].tabList enumerateObjectsUsingBlock:^(CombinationDataModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                                    if (idx == 0) {
                                        ...
                                        if ([kitUtils isBlankString:obj.name]) {
                                            isBlankName = YES;
                                            segmentHeight = 0.f;
                                            newSegmentHeight = 0.f;
                                        }
                                        * stop = YES;
                                    }
                                }];
                                if (isBlankName) {
                                    obj.segmentHeight = 0.f;
                                } else {
                                ...
                                CombinationDataModel *dataObj = obj.data[obj.specialSubjectSelect?:0].tabList[obj.selectData?:0];
                                
                                item = [self getSubIssueItem:dataObj.topics
                                                  selectPage:dataObj.selectPageData?:0];
                                ...
                                [obj.data enumerateObjectsUsingBlock:^(CombinationDataModel * _Nonnull dataModel, NSUInteger idx, BOOL * _Nonnull stop) {
                                    if (idx == 0) {
                                        segmentHeight = [dataModel.name tim_sizeWithMaxWidth:chatLabelMax andFont:TOSKitCustomInfo.shareCustomInfo.chatMessage_tosRobotCombination_segmentFont].height;
                                        
                                        newSegmentHeight = [dataModel.name tim_sizeWithMaxWidth:chateLabelSize.width andFont:TOSKitCustomInfo.shareCustomInfo.chatMessage_tosRobotCombination_segmentFont].height;
            ...
            _bubbleViewF  = CGRectMake(CGRectGetMaxX(_headImageViewF) + headToBubble, cellMargin+topViewH+topViewToBubbleH+topTimeViewH+receiveNickNameToBubbleSpacing+bubbleViewSpecialSubjectAppendY, bubbleSize.width, bubbleSize.height);
            ...
            _specialSubjectF = CGRectMake(CGRectGetMinX(_bubbleViewF), cellMargin+topViewH+topViewToBubbleH+topTimeViewH+receiveNickNameToBubbleSpacing, chatLabelMax + bubblePadding * 2 + arrowWidth, 78.f);
        } else if ([model.message.type isEqualToString:GXRichText]) {
...
//            cardMessgae.extraData = dictionary[@"extraData"];
//            cardMessgae.extraInfoArr = dictionary[@"extraInfo"];
            CGFloat chatOrderWidth = TOSKitCustomInfo.shareCustomInfo.bubbleMaxWidth;
            if (TOSKitCustomInfo.shareCustomInfo.bubbleMaxWidth < 247.0) {
                chatOrderWidth = 247.0f;
            }
            if (chatOrderWidth > (App_Frame_Width-CGRectGetWidth(_headImageViewF)-headToBubble-headToView)) {
                chatOrderWidth = App_Frame_Width-CGRectGetWidth(_headImageViewF)-headToBubble-headToView;
            }
