...
@property (nonatomic, assign) BOOL noCommit;
...
- (void)viewDidLoad {
    ...
    NSString *url = [NSString stringWithFormat:@"%@&source=livechat_leave_message&pageType=inner&authDialog=1%@",self.chatLeaveMessageMsg.ticketPluginUrl,[kitUtils isBlankString:self.chatLeaveMessageMsg.leaveReturnNext] ? @"" : @"&cancelAble=1"];
...
- (void)userContentController:(WKUserContentController *)userContentController
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        NSLog(@"message.name ========== %@，message.body ==== %@， model ==== %@",message.name,message.body,[model yy_modelToJSONObject]);
        /// 提交成功
