...
/// 当前会话的状态
@property (nonatomic, assign) TinetChatStatusType                currentStatusType;
...
//                        [[ICMediaManager sharedManager] saveImage:photo1 msgId:small picType:[self typeForImageData:imageData]];
                    ...
                    default: {
                        ...
                        // TODO: messageF
                        NSString *mediaPath = messageF.model.mediaPath;
                        NSLog(@"获取的媒体路径：%@", mediaPath);
...
//        });
    ...
    [self chatStatusChanged:[[TOSClientKit sharedTOSKit] getCurrentOnlineStatus]];
...
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(updateUnreadMessageNotification:) name:kTIMUpdateUnreadMessageNotification object:nil];
    ...
    /// app的配置信息
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(appSetting:) name:kTIMGetAppSettingNotification object:nil];
...
- (void)updateSessionWindowQuickEntrys:(NSArray <TOSQuickEntryModel *>*)quickEntryAllItems {
    ...
    /// 如果开启了后端的配置，这里的更新就需要拦截下来
    if (model.appWindowSetting.enable) {
        return;
    }
    
    [self updateQuickEntrys:quickEntryAllItems];
    
}

/// 更新快捷入口数据
- (void)updateQuickEntrys:(NSArray <TOSQuickEntryModel *>*)quickEntryAllItems {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        strongself->_quickEntryAllItems = quickEntryAllItems;
        ...
        if (strongself.chatBoxVC.chatBox.textView.text.length) {
            inputTextHeight = strongself.chatBoxVC.chatBox.tosCF_height - inputChatBarHeight;
        }
...
//    [self.tableView registerClass:[OSEventSystemMessageCell class] forCellReuseIdentifier:TypeSystem];
    ...
    /// 订单
    [self.tableView registerClass:[ICChatMessageOrdersCell class] forCellReuseIdentifier:TypeOrderCard];
...
//            }
        ...
        else if ([ID isEqualToString:TypeOrderCard]) {
            ICChatMessageOrdersCell * cell = [tableView dequeueReusableCellWithIdentifier:@"ICChatMessageOrdersCell"];
            if (!cell) {
                cell = [[ICChatMessageOrdersCell alloc] initWithStyle:(UITableViewCellStyleDefault) reuseIdentifier:@"ICChatMessageOrdersCell"];
            }
            cell.modelFrame = modelFrame;
...
//        NSLog(@"留言状态更新点击后的数据：%@", model.dynamicConfigParameters);
//        
...
- (void)quickEntryItemDidTouchModel:(TOSQuickEntryModel *)model {
    ...
    NSLog(@"这里是父类的方法调用");
    /// 工单插件
    if (model.type == TOSQuickEntryEventTypeTicketPlugin) {
        ...
        if ([kitUtils isBlankString:model.dynamicConfigParameters[TOS_TICKET_PLUGIN_URL_RESULT]]) {
            ...
            TOSWorkOrderWebView *workOrderView = [[TOSWorkOrderWebView alloc] initWithFrame:CGRectMake(0, 0, kWindowWidth, kWindowHeight)];
            workOrderView.workOrderUrl = model.dynamicConfigParameters[TOS_TICKET_PLUGIN_URL_RESULT];
            workOrderView.titleStr = model.name;
            UIWindow *window = [UIApplication sharedApplication].keyWindow;
            [window addSubview:workOrderView];
            [workOrderView showPopupView];
            self.workOrderView = workOrderView;
            [[NSNotificationCenter defaultCenter] postNotificationName:kUpdateTicketMessageStatus object:nil];
        }
    ...
    /// 订单抽屉
    if (model.type == TOSQuickEntryEventTypeOrderCard) {
        
        
        [self justOrderVC:model.name withDict:model.params];
        
    }
    /// 发送文本
    if (model.type == TOSQuickEntryEventTypeSendText) {
        if ([[model.params allKeys] containsObject:@"content"]) {
            [self sendText:model.params[@"content"]];
        }
        else {
            [self sendText:model.name];
        }
    }
    /// 转人工
    if (model.type == TOSQuickEntryEventTypeArtificial) {
        [self sendTransferToHumanMessage:model.name];
    }
    /// 满意度
    if (model.type == TOSQuickEntryEventTypeSatisfaction) {
        [self satisfactionEvent];
        
    }
    /// 结束会话
    if (model.type == TOSQuickEntryEventTypeCloseChat) {
...
- (void)satisfactionEvent {
    [[OnlineRequestManager sharedCustomerManager] getInvestigationUniqueIdSuccess:^(NSString * _Nonnull messageUniqueId) {
        ...
        [WHToast showMessage:[NSString stringWithFormat:@"%@",errorDes] duration:2.0 finishHandler:nil];
    }];
}
...
//    if (status == TinetChatStatusTypeOnline) {
//        /// 座席阶段改变，座席的更多面板数据源没有值
//        if (TOSKitChatBoxExtendBoard.shareChatBoxExtendBoard.onlineChange == YES && TOSKitChatBoxExtendBoard.shareChatBoxExtendBoard.onlienAllItems.count == 0) {
//            __weak typeof(self) weakSelf = self;
//            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
//                __strong typeof(weakSelf) strongSelf = weakSelf;
//                [strongSelf.chatBoxVC.chatBox updateFunctionBtn:NO withIndex:2];
//            });
////            dispatch_async(dispatch_get_main_queue(), ^{
////                
////            });
//        }
//        /// 座席阶段改变，座席的更多面板数据源有值
//        if (TOSKitChatBoxExtendBoard.shareChatBoxExtendBoard.onlineChange == YES && TOSKitChatBoxExtendBoard.shareChatBoxExtendBoard.onlienAllItems.count) {
//            __weak typeof(self) weakSelf = self;
//            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
//                __strong typeof(weakSelf) strongSelf = weakSelf;
//                [strongSelf.chatBoxVC.chatBox updateFunctionBtn:YES withIndex:2];
//            });
////            dispatch_async(dispatch_get_main_queue(), ^{
////                __strong typeof(weakSelf) strongSelf = weakSelf;
////                [strongSelf.chatBoxVC.chatBox updateFunctionBtn:YES withIndex:2];
...
/// 触发订单抽屉页面
- (void)chatBoxViewController:(TIMChatBoxViewController *)chatboxViewController orderItem:(ICChatBoxMoreViewItem *)item {
    
    
    NSLog(@"当前的索引：%ld, 总的数组数量：%ld", item.index, [TOSKitChatBoxExtendBoard shareChatBoxExtendBoard].allItems.count);
    if (TOSKitChatBoxExtendBoard.shareChatBoxExtendBoard.onlineChange) {
        if (self.currentStatusType == TinetChatStatusTypeRobot) {
            NSLog(@"进入机器人阶段");
            if (item.index > [TOSKitChatBoxExtendBoard shareChatBoxExtendBoard].allItems.count) {
                return;
            }
            TOSKitExtendBoardItemModel * model = [TOSKitChatBoxExtendBoard shareChatBoxExtendBoard].allItems[item.index];
            [self justOrderVC:model.title withDict:[model dynamicConfigParameters]];
        }
        if (self.currentStatusType == TinetChatStatusTypeOnline) {
            NSLog(@"进入在线阶段");
            if (item.index > TOSKitChatBoxExtendBoard.shareChatBoxExtendBoard.onlienAllItems.count) {
                return;
            }
            TOSKitExtendBoardItemModel * model = [TOSKitChatBoxExtendBoard shareChatBoxExtendBoard].onlienAllItems[item.index];
            [self justOrderVC:model.title withDict:[model dynamicConfigParameters]];
        }
    }
    else {
...
/// 弹出满意度
- (void)chatBoxViewController:(TIMChatBoxViewController *)chatboxViewController withSatisfactionItem:(ICChatBoxMoreViewItem *)satisfactionItem {
    [self satisfactionEvent];
}
...
- (void)chatBoxViewController:(TIMChatBoxViewController *)chatboxViewController withItem:(ICChatBoxMoreViewItem *)item withModel:(TOSKitExtendBoardItemModel *)model {
...
// MARK: app的配置信息
- (void)appSetting:(NSNotification *)notification {
...
/// 配置输入区域的更多面板和快捷入口
- (void)appSettingChatBox {
    TOSSDKConfigModel * model = [TOSSDKConfigModel yy_modelWithJSON:[[OnlineDataSave shareOnlineDataSave] getAppSetting]];
    NSLog(@"进行app的快捷入口和更多面板配置");
    
    if (model.appWindowSetting.enable) {
        if (model.appWindowSetting.quickEntries.count) {
            /// 总的
            NSMutableArray * allQuickArray = [NSMutableArray array];
            /// 座席
            NSMutableArray * onlineQuickArray = [NSMutableArray array];
            
            [model.appWindowSetting.quickEntries enumerateObjectsUsingBlock:^(TOSQuickEntrieModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                TOSQuickEntryModel * quickModel = [[TOSQuickEntryModel alloc] init];
                quickModel.name = obj.name;
                NSDictionary * params = @{
                    @"name": obj.name?:@"",
                    @"content": obj.content?:@"",
                    @"url": obj.url?:@"",
                    ...
                    @"quickEntryTicketPlugin": obj.quickEntryTicketPlugin?:@{},
                    @"searchable": @(obj.searchable)?:@(NO),
                    @"searchPlaceholder": obj.searchPlaceholder?:@"",
                    @"type": @(obj.type),
                };
                quickModel.params = [NSMutableDictionary dictionaryWithDictionary:params];
                switch (obj.type) {
                    case TOSQuickEntrieTypeLink: {
                        quickModel.type = TOSQuickEntryEventTypeLink;
                        break;
                    }
                    case TOSQuickEntrieTypeSatisfaction: {
                        quickModel.type = TOSQuickEntryEventTypeSatisfaction;
                        break;
                    }
                    case TOSQuickEntrieTypeCloseChat: {
                        quickModel.type = TOSQuickEntryEventTypeCloseChat;
                        break;
                    }
                    case TOSQuickEntrieTypeSendText: {
                        quickModel.type = TOSQuickEntryEventTypeSendText;
                        break;
                    }
                    case TOSQuickEntrieTypeArtificial: {
                        quickModel.type = TOSQuickEntryEventTypeArtificial;
                        break;
                    }
                    case TOSQuickEntrieTypeCustom: {
                        quickModel.type = TOSQuickEntryEventTypeCustom;
                        break;
                    }
                ...
                quickModel.value = obj.content;
                /// 留言工单
                if (obj.type == TOSQuickEntrieTypeTicketPlugin) {
                    quickModel.type = TOSQuickEntryEventTypeTicketPlugin;
                    quickModel.dynamicConfigParameters = [NSMutableDictionary dictionary];
                    /// 工单插件url
                    if ([[obj.quickEntryTicketPlugin allKeys] containsObject:@"ticketPluginUrl"]) {
                        [quickModel.dynamicConfigParameters setValue:obj.quickEntryTicketPlugin[@"ticketPluginUrl"] forKey:TOS_TICKET_PLUGIN_URL];
                    }
                    /// 获取工单客服留言数量开关
                    if ([[obj.quickEntryTicketPlugin allKeys] containsObject:@"commentCountEnable"]) {
                        [quickModel.dynamicConfigParameters setValue:obj.quickEntryTicketPlugin[@"commentCountEnable"] forKey:TOS_COMMENT_COUNT_ENABLE];
                    }
//                    /// 工单插件id，之前逻辑没有用到，这里就不取了。
//                    if ([[obj.quickEntryTicketPlugin allKeys] containsObject:@"ticketPluginId"]) {
//
//                    }
                    /// 隐藏入口
                    if ([[obj.quickEntryTicketPlugin allKeys] containsObject:@"hideEntry"]) {
                        [quickModel.dynamicConfigParameters setValue:obj.quickEntryTicketPlugin[@"hideEntry"] forKey:TOS_NO_COMMENT_COUNT_HIDE_QUICK_ENTRY];
                    }
                    /// 仅查询和展示访客创建的工单
                    if ([[obj.quickEntryTicketPlugin allKeys] containsObject:@"visitorCreatedTicket"]) {
                        [quickModel.dynamicConfigParameters setValue:obj.quickEntryTicketPlugin[@"visitorCreatedTicket"] forKey:TOS_VISITOR_CREATED_TICKET];
                    }
                    /// 留言工单的显示阶段
                    if (obj.quickEntryRobotEnable && obj.quickEntryAgentEnable) {
                        quickModel.dynamicConfigParameters[TOS_APPLICATION_STAGE] = TOSAppLicationStageType_Both;
                    } else if (obj.quickEntryRobotEnable) {
                        quickModel.dynamicConfigParameters[TOS_APPLICATION_STAGE] = TOSAppLicationStageType_Robot;
                    } else if (obj.quickEntryAgentEnable) {
                        quickModel.dynamicConfigParameters[TOS_APPLICATION_STAGE] = TOSAppLicationStageType_Human;
                    } else {
                        quickModel.dynamicConfigParameters[TOS_APPLICATION_STAGE] = TOSAppLicationStageType_OFF;
                    }
                    /// 工单的数量，默认是 0 ，跟TOS_TICKET_PLUGIN_URL_RESULT的数据都是通过服务器返回具体的数量
                    [quickModel.dynamicConfigParameters setValue:@"0" forKey:TOS_STAFF_COMMENT_TOTAL_COUNT];
                    /// 初始化为空，通过 TOS_TICKET_PLUGIN_URL 获取真实的工单地址
                    [quickModel.dynamicConfigParameters setValue:@"" forKey:TOS_TICKET_PLUGIN_URL_RESULT];
                    
                }
                /// 订单卡片
                if (obj.type == TOSQuickEntrieTypeOrderCard) {
                    quickModel.type = TOSQuickEntryEventTypeOrderCard;
                    NSDictionary * params = @{
                        @"content" : obj.content?:@"",
                        @"url" : obj.url?:@"",
                        @"param" : obj.param?:@"",
                        ...
                        @"searchPlaceholder" : obj.searchPlaceholder?:@"",
                        @"categoryDisplay" : obj.categoryDisplay?:@(NO),
                        @"categoryList" : obj.categoryList?:@[],
                    };
                    quickModel.params = [NSMutableDictionary dictionaryWithDictionary:params];
                    
                }
                if (obj.quickEntryRobotEnable) {
                    [allQuickArray addObject:quickModel];
                }
                if (obj.quickEntryAgentEnable) {
                    [onlineQuickArray addObject:quickModel];
                }
                
            }];
            if (self.currentStatusType == TinetChatStatusTypeRobot) {
                NSLog(@"allQuickArray的数量：%ld", allQuickArray.count);
                [self updateQuickEntrys:allQuickArray];
            } else if (self.currentStatusType == TinetChatStatusTypeOnline) {
                NSLog(@"onlineQuickArray的数量：%ld", onlineQuickArray.count);
                [self updateQuickEntrys:onlineQuickArray];
            } else {
                [self updateQuickEntrys:@[]];
            }
            
            
//            NSLog(@"后端接口更新快捷入口的配置:%@", quickArray);
        ...
        if (model.appWindowSetting.toolbarList.count) {
            /// 总的
            NSMutableArray * toolBarArray = [NSMutableArray array];
            /// 座席
            NSMutableArray * onlineBarArray = [NSMutableArray array];
            
            /// 是否区分座席阶段和非座席阶段
            __block BOOL onlineChange = YES;
            
            NSInteger index = 0;
            
            for (TOSToolbarItemModel * obj in model.appWindowSetting.toolbarList) {
                TOSKitExtendBoardItemModel * moreModel = [[TOSKitExtendBoardItemModel alloc] init];
                moreModel.title = obj.name;
                moreModel.imageURL = obj.iconUrl;
                moreModel.index = index;
                if (obj.type == TOSToolbarItemEventTypePhoto) {
                    moreModel.type = TOSChatBoxExtendBoardTypePhotos;
                }
                else if (obj.type == TOSToolbarItemEventTypeTakePicture) {
                    moreModel.type = TOSChatBoxExtendBoardTypeTakePicture;
                }
                else if (obj.type == TOSToolbarItemEventTypeFile) {
                    if (obj.fileSource == 1) {
                        moreModel.type = TOSChatBoxExtendBoardTypeCustomFile;
                    }
                    else {
                        moreModel.type = TOSChatBoxExtendBoardTypeCustomFileApp;
                    }
                }
                else if (obj.type == TOSToolbarItemEventTypeArtificial) {
                    moreModel.type = TOSChatBoxExtendBoardTypeArtificial;
                }
                else if (obj.type == TOSToolbarItemEventTypeCloseChat) {
                    moreModel.type = TOSChatBoxExtendBoardTypeCloseChat;
                }
                else if (obj.type == TOSToolbarItemEventTypeCustom) {
                    moreModel.type = TOSChatBoxExtendBoardTypeCustom;
                }
                else if (obj.type == TOSToolbarItemEventTypeLink) {
                    moreModel.type = TOSChatBoxExtendBoardTypeLink;
                    moreModel.dynamicConfigParameters = [NSMutableDictionary dictionaryWithDictionary:@{@"link": obj.url}];
                    //                    NSLog(@"外链的地址：%@", moreModel.dynamicConfigParameters);
                    
                }
                else if (obj.type == TOSToolbarItemEventTypeOrderCard) {
                    moreModel.type = TOSChatBoxExtendBoardTypeOrder;
                    NSDictionary * params = @{
                        @"content" : @"",
                        @"url" : obj.url?:@"",
                        @"param" : obj.param?:@"",
                    ...
                    moreModel.dynamicConfigParameters = [NSMutableDictionary dictionaryWithDictionary:params];
                }
                else if (obj.type == TOSToolbarItemEventTypeSatisfaction) {
                    moreModel.type = TOSChatBoxExtendBoardTypeSatisfaction;
                }
                if (obj.stageEnable == 0) {
                    onlineChange = NO;
                }
                else if (obj.stageEnable == 1) {
                    [toolBarArray addObject:moreModel];
                    index++;
                }
                else if (obj.stageEnable == 3) {
                    [toolBarArray addObject:moreModel];
                    index++;
                }
            ...
            for (TOSToolbarItemModel * obj in model.appWindowSetting.toolbarList) {
                ...
                else if (obj.stageEnable == 2) {
                    [onlineBarArray addObject:moreModel];
                    index++;
                }
                else if (obj.stageEnable == 3) {
                    [onlineBarArray addObject:moreModel];
...
//            [model.appWindowSetting.toolbarList enumerateObjectsUsingBlock:^(TOSToolbarItemModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
//                TOSKitExtendBoardItemModel * moreModel = [[TOSKitExtendBoardItemModel alloc] init];
//                moreModel.title = obj.name;
//                moreModel.imageURL = obj.iconUrl;
//                moreModel.index = idx;
...
//            }];
            ...
            TOSKitChatBoxExtendBoard.shareChatBoxExtendBoard.allItems = toolBarArray;
            TOSKitChatBoxExtendBoard.shareChatBoxExtendBoard.onlienAllItems = onlineBarArray;
            
//            NSLog(@"全部的：%@   座席阶段的：%@", toolBarArray, onlineBarArray);
//            NSLog(@"当前的状态是：%lu", (unsigned long)self.currentStatusType);
            /// 当前是座席阶段，座席数组为空。需要隐藏
            if (onlineChange && onlineBarArray.count == 0 && self.currentStatusType == TinetChatStatusTypeOnline) {
                __weak typeof(self) weakSelf = self;
                dispatch_async(dispatch_get_main_queue(), ^{
                    __strong typeof(weakSelf) strongSelf = weakSelf;
                    [strongSelf.chatBoxVC.chatBox updateFunctionBtn:NO withIndex:2];
            ...
            else if (toolBarArray.count == 0 && (self.currentStatusType == TinetChatStatusTypeRobot || self.currentStatusType == TinetChatStatusTypeOutline)) {
                __weak typeof(self) weakSelf = self;
                dispatch_async(dispatch_get_main_queue(), ^{
                    __strong typeof(weakSelf) strongSelf = weakSelf;
                    [strongSelf.chatBoxVC.chatBox updateFunctionBtn:NO withIndex:2];
            ...
            [combinationMessage enumerateObjectsUsingBlock:^(CombinationMessage * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                ...
                if (obj.type.integerValue == 36) {
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        ...
                        TOSRobotFormView *robotFormView = [[TOSRobotFormView alloc] initWithFrame:CGRectMake(0, 0, kWindowWidth, kWindowHeight)];
                        robotFormView.robotFormData = obj.robotFormData;
                        UIWindow *window = [UIApplication sharedApplication].keyWindow;
                        [window addSubview:robotFormView];
...
//            }];
            ...
            [readyDisplayDataSource enumerateObjectsUsingBlock:^(TIMMessageFrame *objModel, NSUInteger idx, BOOL * _Nonnull stop) {
                ...
                if ([objModel.model.message.type isEqualToString:TypeEventQueue]) {
                    objModel.model.message.hideLeaveQueueBtn = YES;
    ...
    //        [self messageSendSucced:messageF];
        ...
        else if ([message.type isEqualToString:@"TIMOrderCard"]) {
            TOSOrderMessage * orderMessage = (TOSOrderMessage *)message.content;
            TIMMessageFrame *messageF = [ICMessageHelper createMessageFrame:TypeOrderCard originalType:TypeOrderCard content:(NSString *)message.content extra:@"" auditExtra:@"" path:nil urlPath:nil from:message.senderId to:message.receiverId fileKey:message.messageUUID bigImgFileId:@"" voiceDuration:[NSNumber numberWithInt:0] msgId:message.msg_id sendTime:message.timestamp showTime:[self p_needShowTimeStamp:(NSTimeInterval)message.timestamp] picSize:CGSizeMake(0, 0) picType:@"" isSender:NO receivedSenderByYourself:NO status:1 senderType:[NSNumber numberWithInteger:message.senderType.integerValue]];
            [self addObject:messageF isSender:NO isHeaderInsert:NO];
            [self messageSendSucced:messageF];
        }
        else if ([message.type isEqualToString:@"ordertrigger"]) {
            TIMOrderEventMessage * eventMessage = (TIMOrderEventMessage *)message.content;
            /// 快捷入口
            if ([eventMessage.event isEqualToString:@"triggerQuickAccess"]) {
                __block BOOL showOrder = NO;
                __block TOSQuickEntryModel * quickModel = nil;
                [self.chatBoxVC.barItemArray enumerateObjectsUsingBlock:^(TOSQuickEntryModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    if (obj.type == TOSQuickEntryEventTypeOrderCard &&
                        [obj.name isEqualToString:eventMessage.name]) {
                        quickModel = obj;
                        showOrder = YES;
                        *stop = YES;
                    }
                ...
                if (showOrder) {
                    [quickModel.params setValue:eventMessage.failContent forKey:@"failContent"];
                    [self justOrderVC:eventMessage.name withDict:quickModel.params];
                } else {
                    if (eventMessage.failContent.length) {
                        [self sendText:eventMessage.failContent];
                    }
                }
                
            }
            /// 更多面板
            if ([eventMessage.event isEqualToString:@"triggerToolBar"]) {
                /// 默认加载扩展面板的item数据源
                ...
                __block BOOL showOrder = NO;
                __block TOSKitExtendBoardItemModel * moreModel = nil;
                [allItems enumerateObjectsUsingBlock:^(TOSKitExtendBoardItemModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    if (obj.type == TOSChatBoxExtendBoardTypeOrder &&
                        [obj.title isEqualToString:eventMessage.name]) {
                        moreModel = obj;
                        showOrder = YES;
                        *stop = YES;
                    }
                ...
                if (showOrder) {
                    [moreModel.dynamicConfigParameters setValue:eventMessage.failContent forKey:@"failContent"];
                    [self justOrderVC:eventMessage.name withDict:moreModel.dynamicConfigParameters];
                } else {
                    if (eventMessage.failContent.length) {
                        [self sendText:eventMessage.failContent];
                    }
...
/// 跳转到订单抽屉页面
- (void)justOrderVC:(NSString *)title withDict:(NSDictionary *)param {
...
//    NSLog(@"当前的参数为：%@", param);
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        __strong typeof(weakself) strongself = weakself;
        TOSOrderViewController * orderVC = [[TOSOrderViewController alloc] init];
        if ([[param allKeys] containsObject:@"searchable"]) {
            NSNumber * searchAnble = param[@"searchable"];
            orderVC.showSearch = [searchAnble boolValue];
        }
        if ([[param allKeys] containsObject:@"searchPlaceholder"]) {
            orderVC.searchPlaceholder = param[@"searchPlaceholder"];
        }
        /// 有失败话术，则说明是被动唤起
        if ([[param allKeys] containsObject:@"failContent"]) {
            orderVC.passiveEvoke = YES;
        }
        
        orderVC.titleStr = @"请选择您要查询的订单";
        orderVC.param = param;
        orderVC.delegate = strongself;
        UINavigationController * navigationVC = [[UINavigationController alloc] initWithRootViewController:orderVC];
        /// ⚠️⚠️⚠️
        /// 这里必须设置style样式为UIModalPresentationOverFullScreen，不然是看不到上一级页面的内容，后续要改动这里是要慎重
        /// ⚠️⚠️⚠️
        navigationVC.modalPresentationStyle = UIModalPresentationOverFullScreen;
        [strongself presentViewController:navigationVC animated:YES completion:nil];
...
#pragma mark - TOSOrderViewControllerDelegate
/// 发送订单卡片
/// - Parameters:
///   - cardModel: 订单model
///   - productModel: 商品model
///   - type: 枚举，是发送订单还是发送商品
- (void)TOSOrderViewControllerSendCard:(TOSOrderListModel *)cardModel withProductModel:(TOSOrderListProductModel *)productModel withType:(TOSOrderModelType)type {
    /// 发送商品
    NSMutableDictionary * params = [NSMutableDictionary dictionary];
    /// 发送订单
    if (type == TOSOrderModelTypeOrder) {
        if (cardModel.extraData) {
            
            [params setValue:cardModel.extraData forKey:@"extraData"];
        }
        else {
            [params setValue:@[] forKey:@"extraData"];
        }
        if (cardModel.extraInfoArr) {
            
            [params setValue:cardModel.extraInfoArr forKey:@"extraInfo"];
        }
        else {
            [params setValue:@[] forKey:@"extraInfo"];
        }
        
        NSMutableArray * productArray = [NSMutableArray array];
        [cardModel.productList enumerateObjectsUsingBlock:^(TOSOrderListProductModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            NSMutableArray * buttonConfigListArr = [NSMutableArray array];
            for (TOSOrderButtonConfigModel * configModel in obj.buttonConfigList) {
                [buttonConfigListArr addObject:[configModel dictionaryRepresentation]];
            }
            NSDictionary * product = @{
                ...
                @"img": obj.img?:@"",
                @"imgTag": obj.imgTag?:@"",
                @"imgTagStyle": obj.imgTagStyle?:@"",
                @"visitorUrl": obj.visitorUrl?:@"",
                @"agentUrl": obj.agentUrl?:@"",
                @"appId": obj.appId?:@"",
                @"status": obj.status?:@"",
                @"pagePath": obj.pagePath?:@"",
                @"extraData": obj.extraData?:@[],
                @"buttonConfigList": buttonConfigListArr?:@[]
            };
            [productArray addObject:product];
        }];
        
        NSMutableArray * buttonConfigList = [NSMutableArray array];
        
        
        for (TOSOrderButtonConfigModel * btnConfig in cardModel.buttonConfigList) {
            [buttonConfigList addObject:[btnConfig dictionaryRepresentation]];
        }
        
        NSDictionary * bottomButtonConfig = [cardModel.bottomButtonConfig dictionaryRepresentation];
        /// 订单信息
        NSMutableDictionary * orderInfo = [NSMutableDictionary dictionary];
        if (productArray.count) {
            [orderInfo setValue:productArray forKey:@"productList"];
        }
        [orderInfo setValue:cardModel.title?:@"" forKey:@"title"];
        [orderInfo setValue:cardModel.logo?:@"" forKey:@"logo"];
        [orderInfo setValue:cardModel.time?:@"" forKey:@"time"];
        [orderInfo setValue:cardModel.number?:@"" forKey:@"number"];
        [orderInfo setValue:cardModel.status?:@"" forKey:@"status"];
        [orderInfo setValue:buttonConfigList?:@[] forKey:@"buttonConfigList"];
        if ([bottomButtonConfig allKeys].count) {
            [orderInfo setValue:bottomButtonConfig forKey:@"bottomButtonConfig"];
        }
        [orderInfo setValue:cardModel.extraData?:@[] forKey:@"extraData"];
        [orderInfo setValue:cardModel.extraInfoArr?:@[] forKey:@"extraInfo"];
        [orderInfo setValue:cardModel.tag?:@{} forKey:@"tag"];
        
//        NSString * orderInfoStr = [orderInfo yy_modelToJSONString];
        [params setValue:orderInfo forKey:@"orderInfo"];
//        NSString * content = [params yy_modelToJSONString];
//        NSLog(@"订单的发送内容：%@", params);
        [self sendOrderCard:params];
    }
    else {
        
        if (productModel.extraData) {
            
            [params setValue:productModel.extraData forKey:@"extraData"];
        }
        else {
            [params setValue:@[] forKey:@"extraData"];
        }
        [params setValue:@[] forKey:@"extraInfo"];
        NSMutableArray * buttonConfigListArr = [NSMutableArray array];
        for (TOSOrderButtonConfigModel * configModel in productModel.buttonConfigList) {
            [buttonConfigListArr addObject:[configModel dictionaryRepresentation]];
        }
        ...
        NSDictionary * product = @{
            ...
            @"imgTag": productModel.imgTag?:@"",
            @"imgTagStyle": productModel.imgTagStyle?:@"",
            @"visitorUrl": productModel.visitorUrl?:@"",
            @"agentUrl": productModel.agentUrl?:@"",
            @"appId": productModel.appId?:@"",
            @"status": productModel.status?:@"",
            @"pagePath": productModel.pagePath?:@"",
            @"extraData": productModel.extraData?:@[],
            @"buttonConfigList": buttonConfigListArr?:@""
        };
        NSDictionary * orderInfo = @{@"productList" : @[product]};
        
//        NSString * orderInfoStr = [orderInfo yy_modelToJSONString];
        [params setValue:orderInfo forKey:@"orderInfo"];
//        NSString * content = [params yy_modelToJSONString];
//        NSLog(@"商品的发送内容：%@", params);
        [self sendOrderCard:params];
    }
...
- (void)TOSOrderViewControllerSendContent:(NSString *)content withCard:(TOSOrderListModel *)cardModel {
    [self sendTextMessagewithText:content knowledge:@"" sysTransfer:NO];
}
...
//    /// 发布时需要移除。这里只是用于给测试看的
//    [WHToast showMessage:link duration:2.0 finishHandler:nil];
    
    STBaseWebViewController * webVC = [[STBaseWebViewController alloc] init];
    webVC.urlString = link;
    [self.navigationController pushViewController:webVC animated:YES];
    
    [self routerEventWithName:TinetRouterClickOrderCardEvent userInfo:@{@"content": [cardModel yy_modelToJSONObject]}];
...
- (void)TOSOrderViewControllerSendFailText:(NSString *)failText {
    /// 失败话术不为空进行消息的发送
    if (failText.length) {
        [self sendTextMessagewithText:failText knowledge:@"" sysTransfer:NO];
    }
...
- (void)TOSOrderViewControllerTouchProductCard:(TOSOrderListProductModel *)cardModel withOrderModel:(TOSOrderListModel *)orderModel {
    
    [self routerEventWithName:TinetRouterClickOrderCardEvent userInfo:@{
        @"content": @{
            @"productModel": [cardModel yy_modelToJSONObject],
            @"orderModel": [orderModel yy_modelToJSONObject]
        },
    }];
}

/// 发送订单卡片
- (void)sendOrderCard:(NSDictionary *)card {
    
    
    @WeakObj(self);
    [self.reloadTaskManager startTaskWithScroll:YES completion:^{
        @StrongObj(self);
        TOSOrderMessage * orderMessage = [TOSOrderMessage yy_modelWithJSON:card[@"orderInfo"]];
        orderMessage.extraData = card[@"extraData"];
        orderMessage.extraInfoArr = card[@"extraInfo"];
        // Kit
        __block TIMMessageFrame *messageF = [ICMessageHelper createMessageFrame:TypeOrderCard
                                                                   originalType:TypeOrderCard
                                                                        content:(NSString *)orderMessage
                                                                          extra:[card yy_modelToJSONString]
                                                                     auditExtra:@""
                                                                           path:nil
                                                                        urlPath:nil
                                                                           from:[[OnlineDataSave shareOnlineDataSave]getVisitorId]
                                                       ...
                                                       receivedSenderByYourself:NO
                                                                         status:TIMMessageStatus_Sending
                                                                     senderType:@(2)];
        messageF.model.message.knowledge = @"";
        messageF.model.message.intent = @"";
        messageF.model.message.sysTransfer = NO;
    ...
    //            self.chatBoxVC.view.hidden = NO;
                ...
                self.offlineType = offlineMessageTypeOrder;
                self.offlineNewMessage = messageF;
            ...
            [[OnlineMessageSendManager sharedOnlineMessageSendManager] sendOrderCardMessageWithMessageStr:card success:^(TOSMessage * _Nonnull timMessage) {
                @StrongObj(self);
                messageF.model.message.messageId = timMessage.msg_id;
                [self updateOldMessageId:timMessage.msg_id];
                [self messageSendSucced:messageF];
...
- (void)reSendMessage:(ICChatMessageBaseCell *)baseCell {
    ...
    if (![[TOSClientKit sharedTOSKit].mqttConnected boolValue]) {
        [WHToast showImage:[UIImage imageNamed:[NSString stringWithFormat:@"%@/%@", FRAMEWORKS_BUNDLE_PATH, @"reSend_tipIcon"]] message:@"网络异常" duration:2.0 finishHandler:nil];
        return;
    }
...
//            NSLog(@"视频媒体资源路径：%@", mediaPath);
            NSURL *fileURL = [NSURL URLWithString:mediaPath];
            NSError *error = nil;
            NSData * videoData = [NSData dataWithContentsOfURL:fileURL options:NSDataReadingMappedIfSafe error:&error];
//            if (videoData) {
//                NSLog(@"视频数据大小：%@", videoData);
//            }
//            else {
//                NSLog(@"视频数据大小为空");
//            }
//            NSData *videoData = [NSData dataWithContentsOfFile:mediaPath];
            ...
            if (![mediaPath hasPrefix:@"file://"]) {
                fileURL = [NSURL fileURLWithPath:mediaPath];
            }
            NSError *error = nil;
            NSData * fileData = [NSData dataWithContentsOfURL:fileURL options:NSDataReadingMappedIfSafe error:&error];
            if (error) {
//                NSLog(@"文件转data格式error信息：%@", error);
                if (error.code == 260) {
                    fileURL = [NSURL fileURLWithPath:modelFrame.model.urlPath];
                    fileData = [NSData dataWithContentsOfURL:fileURL options:NSDataReadingMappedIfSafe error:&error];
                }
        ...
        else if ([message.type isEqualToString:TypeOrderCard]) {
//            NSLog(@"message.content : %@", message.extra);
            TOSOrderMessage * orderMessage = (TOSOrderMessage *)message.extra;
            NSDictionary * card = @{
                @"extraData" : orderMessage.extraData?:@[],
                @"extraInfo": orderMessage.extraInfoArr?:@[],
                @"orderInfo" : [orderMessage yy_modelToJSONObject]
            };
//            NSLog(@"重发的商品卡片信息：%@", card);
            [[OnlineMessageSendManager sharedOnlineMessageSendManager] sendOrderCardMessageWithMessageStr:card success:^(TOSMessage * _Nonnull timMessage) {
                @StrongObj(self);
                modelFrame.model.message.messageId = timMessage.msg_id;
                [self updateOldMessageId:timMessage.msg_id];
                [self messageSendSucced:modelFrame];
                self.reloadTaskManager.isScroll = YES;
                [self.reloadTaskManager startCountdown];
...
- (void)tinet_textMessageClickAction:(TinetClickTextMessageEventType)eventType userInfo:(NSDictionary *)userInfo {
    ...
    if (eventType == TinetClickEventTypeUrl) {
        if ([userInfo tim_ObjectForKey:@"content"] &&
            ...
            BOOL isOpen = [[UIApplication sharedApplication] openURL:[NSURL URLWithString:userInfo[@"content"]]];
            if (!isOpen) {
...
- (void)sendCard:(TOSClientKitCommodityCardOption *)userInfo {
    ...
    if (userInfo.extraData) {
        ...
        [mutableDic setObject:@[] forKey:@"extraData"];
    }
                   ...
                   userInfo:(NSDictionary *)userInfo
{
    ...
    if ([eventName isEqualToString:GXRouterEventTextUrlTapEventName]) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            self.popupView = [[TOSSatisfactionPopupView alloc] initWithFrame:CGRectMake(0, 0, kWindowWidth, kWindowHeight)];
        ...
        if ([modelFrame.model.message.content isKindOfClass:[NSArray class]]) {
            
            NSArray *array = (NSArray *)modelFrame.model.message.content;
            if (array &&
                array.count > 0 &&
                [array.firstObject isKindOfClass:[CombinationMessage class]]) {
                
                [array enumerateObjectsUsingBlock:^(CombinationMessage *obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    @strongify(self)
                    if (obj.clickLikeFeature) {
                        
                        NSArray *readyDisplayDataSource = [self.readyDisplayDataSource mutableCopy];
                        if (readyDisplayDataSource &&
                            readyDisplayDataSource.count > 0 &&
                            [readyDisplayDataSource.lastObject isEqual:modelFrame]) {
                            isScroll = YES;
                        }
                        *stop = YES;
                    }
        ...
        if ([ICFileTool fileExistsAtPath:orgImgPath]) {
            ...
            imageVC.animator.originImageView = [userInfo objectForKey:@"currentImage"];
            imageVC.animator.targetImageView = imageVC.imageView;
        ...
        [WHToast showMessage:@"暂不支持查看关联知识" duration:2.0 finishHandler:nil];
...
//        [self sendTextMessagewithText:message knowledge:knowledge sysTransfer:NO];
    ...
    else if ([eventName isEqualToString:TinetRouterClickOrderCardEvent]) {
        if ([self respondsToSelector:@selector(tinet_textMessageClickAction:userInfo:)]) {
            [self tinet_textMessageClickAction:(TinetClickOrderCard) userInfo:userInfo];
        }
    }
...
//        messageStr = model.content;
    ...
    else if ([model.messageType intValue] == 38) {
        ...
        NSDictionary *jsonDict = [NSJSONSerialization JSONObjectWithData:jsonData
                                                           options:NSJSONReadingFragmentsAllowed
                                                             error:&err];
//        NSLog(@"解析的字典的值：%@", jsonDict);
//        NSLog(@"JSON parsing error: %@", err);
        if ([jsonDict allKeys].count > 0 && [jsonDict objectForKey:@"orderInfo"]) {
//            NSDictionary * contentDict = jsonDict[@"content"];
            TOSOrderMessage * orderMessage = [TOSOrderMessage yy_modelWithJSON:jsonDict[@"orderInfo"]];
            orderMessage.extraData = jsonDict[@"extraData"];
            orderMessage.extraInfoArr = jsonDict[@"extraInfo"];
            TIMMessageFrame *messageF = [ICMessageHelper createMessageFrame:TypeOrderCard originalType:TypeOrderCard content:(NSString *)orderMessage extra:model.content auditExtra:@"" path:nil urlPath:nil from:model.sender to:model.visitorId fileKey:model.fileKey bigImgFileId:@"" voiceDuration:[NSNumber numberWithInt:0] msgId:model.uniqueId sendTime:[model.createTime  doubleValue] showTime:[self p_needShowTimeStamp:(NSTimeInterval)[model.createTime  doubleValue]] picSize:CGSizeMake(0, 0) picType:@"" isSender:!messageRecvDirection receivedSenderByYourself:NO status:1 senderType:model.senderType];
            [self addObject:messageF isSender:NO];
            
//            NSLog(@"历史的 model.message.type ==== :%@", messageF.model.message.type);
            if (reload == NO) {
                return;
            }
            [self messageSendSucced:messageF];
...
- (void)reloadTableViewWithFlash{
    ...
    if (self.dataSource.count > 0 &&
        ...
        @weakify(self);
        if (@available(iOS 11.0, *)) {
            [self.tableView performBatchUpdates:^{} completion:^(BOOL finished) {
                @strongify(self);
                [self.tableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:self.dataSource.count-1 inSection:0] atScrollPosition:UITableViewScrollPositionBottom animated:YES];
                if (self.firstHasGetHistoryListFinish == 1) {
                    [self addCommodityCardMessage];
