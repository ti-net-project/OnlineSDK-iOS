...
- (ICChatBoxMoreViewItem *)setupxMoreItem:(NSString *)title
    ...
    if (obj.imageURL.length) {
        return [ICChatBoxMoreViewItem createChatBoxMoreItemWithTitle:obj.title imageURL:obj.imageURL index:obj.index type:obj.type];
    }
    ...
    if (allItems.count) {
        if (self.chatBox) {
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                ...
                [strongSelf.chatBox updateFunctionBtn:YES withIndex:2];
            });
        }
        ...
        if (self.chatBox) {
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                __strong typeof(weakSelf) strongSelf = weakSelf;
                [strongSelf.chatBox updateFunctionBtn:NO withIndex:2];
            });
        }
    }
    ...
    [allItems enumerateObjectsUsingBlock:^(TOSKitExtendBoardItemModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        switch (obj.type) {
            ...
            case TOSChatBoxExtendBoardTypeCustomFileApp: {
                ...
                break;
            case TOSChatBoxExtendBoardTypeOrder: {
                ICChatBoxMoreViewItem *closeChatItem = [self setupxMoreItem:@"订单" imageName:@"more_order" obj:obj];
                [arr addObject:closeChatItem];
                break;
            }
            case TOSChatBoxExtendBoardTypeLink: {
                ICChatBoxMoreViewItem *closeChatItem = [self setupxMoreItem:@"链接" imageName:@"onlineLink" obj:obj];
                [arr addObject:closeChatItem];
                break;
            }
            case TOSChatBoxExtendBoardTypeCustom: {
                ICChatBoxMoreViewItem *closeChatItem = [self setupxMoreItem:@"自定义" imageName:@"onlineCustom" obj:obj];
                [arr addObject:closeChatItem];
                break;
            }
            case TOSChatBoxExtendBoardTypeSatisfaction: {
                ICChatBoxMoreViewItem *closeChatItem = [self setupxMoreItem:@"满意度" imageName:@"onlineSatisfaction" obj:obj];
                [arr addObject:closeChatItem];
                break;
            }
            default: {
...
- (void)ICChatBoxBarWithItemDidselectedIndex:(NSInteger)index withModel:(TOSQuickEntryModel *)model {
    if (self.delegate && [self.delegate respondsToSelector:@selector(chatBoxDidClickBarItemViewController:withItemModel:)]) {
        [self.delegate chatBoxDidClickBarItemViewController:self withItemModel:model];
    }
}
...
- (void) chatBoxMoreView:(ICChatBoxMoreView *)chatBoxMoreView
    ...
    switch (item.type) {
        ...
        case TOSChatBoxExtendBoardTypeOrder: {
            if (self.delegate &&
                [self.delegate respondsToSelector:@selector(chatBoxViewController:orderItem:)]) {
                [self.delegate chatBoxViewController:self orderItem:item];
            }
            break;
        }
        case TOSChatBoxExtendBoardTypeSatisfaction: {
            if (self.delegate &&
                [self.delegate respondsToSelector:@selector(chatBoxViewController:withSatisfactionItem:)]) {
                [self.delegate chatBoxViewController:self withSatisfactionItem:item];
            }
            break;
        }
        case TOSChatBoxExtendBoardTypeLink: {
            if (self.delegate && [self.delegate respondsToSelector:@selector(chatBoxViewController:withItem:withModel:)]) {
                /// 默认加载扩展面板的item数据源
                NSArray <TOSKitExtendBoardItemModel *>*allItems = [TOSKitChatBoxExtendBoard shareChatBoxExtendBoard].allItems;
                /// 如果开启了座席在线状态下修改更多面板的item开关，并且当前的会话状态是座席接待，替换扩展面板的item数据源
                if (TOSKitChatBoxExtendBoard.shareChatBoxExtendBoard.onlineChange && TOSClientKit.sharedTOSKit.getCurrentOnlineStatus == TinetChatStatusTypeOnline) {
                ...
                __block TOSKitExtendBoardItemModel * testModel = nil;
                [allItems enumerateObjectsUsingBlock:^(TOSKitExtendBoardItemModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    if (obj.title == itemTitle) {
                        testModel = obj;
                        *stop = YES;
                    }
                }];
                [self.delegate chatBoxViewController:self withItem:item withModel:testModel];
            }
            break;
        }
        default: {
