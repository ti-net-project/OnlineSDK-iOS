...
@interface TIMYYTextView () <UIScrollViewDelegate, UIAlertViewDelegate, TIMYYTextDebugTarget, YYTextKeyboardObserver> {
    ...
    TIMYYTextContainer *_innerContainer; ///< nonnull, inner text container
    TIMYYTextLayout *_innerLayout; ///< inner text layout, the text in this layout is longer than `_innerText` by appending '\n'
    
    TIMYYTextContainerView *_containerView; ///< nonnull
    TIMYYTextSelectionView *_selectionView; ///< nonnull
    TIMYYTextMagnifier *_magnifierCaret; ///< nonnull
    TIMYYTextMagnifier *_magnifierRanged; ///< nonnull
    ...
    TIMYYTextHighlight *_highlight; ///< highlight attribute in `_highlightRange`
    TIMYYTextLayout *_highlightLayout; ///< when _state.showingHighlight=YES, this layout should be displayed
    TIMYYTextRange *_trackingRange; ///< the range in _innerLayout, may out of _innerText.
...
- (void)_commitUpdate {
    ...
    [[TIMYYTransaction transactionWithTarget:self selector:@selector(_updateIfNeeded)] commit];
#else
    [self _update];
#endif
}
...
- (void)_updateLayout {
    ...
    _innerLayout = [TIMYYTextLayout layoutWithContainer:_innerContainer text:text];
...
- (void)_updateSelectionView {
    ...
    [[TIMYYTextEffectWindow sharedWindow] hideSelectionDot:_selectionView];
    if (!_innerLayout) return;
    
    NSMutableArray *allRects = [NSMutableArray new];
    BOOL containsDot = NO;
    
    TIMYYTextRange *selectedRange = _selectedTextRange;
    ...
    [allRects enumerateObjectsUsingBlock:^(TIMYYTextSelectionRect *rect, NSUInteger idx, BOOL *stop) {
    ...
    if (!_state.firstShowDot && containsDot) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.02 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [[TIMYYTextEffectWindow sharedWindow] showSelectionDot:_selectionView];
        });
    }
    [[TIMYYTextEffectWindow sharedWindow] showSelectionDot:_selectionView];
...
- (void)_commitPlaceholderUpdate {
    ...
    [[TIMYYTransaction transactionWithTarget:self selector:@selector(_updatePlaceholderIfNeeded)] commit];
#else
...
- (void)_updatePlaceholder {
    ...
    if (_placeholderAttributedText.length > 0) {
        TIMYYTextContainer *container = _innerContainer.copy;
        container.size = self.bounds.size;
        container.truncationType = YYTextTruncationTypeEnd;
        container.truncationToken = nil;
        TIMYYTextLayout *layout = [TIMYYTextLayout layoutWithContainer:container text:_placeholderAttributedText];
...
- (void)_updateTextRangeByTrackingCaret {
    ...
    TIMYYTextPosition *newPos = [_innerLayout closestPositionToPoint:trackingPoint];
    if (newPos) {
        ...
        TIMYYTextRange *newRange = [TIMYYTextRange rangeWithRange:NSMakeRange(newPos.offset, 0) affinity:newPos.affinity];
...
- (void)_updateTextRangeByTrackingGrabber {
    ...
    TIMYYTextPosition *position = [_innerLayout positionForPoint:magPoint
    ...
    if (position) {
        ...
        if ((NSUInteger)position.offset > _innerText.length) {
            position = [TIMYYTextPosition positionWithOffset:_innerText.length];
        }
        TIMYYTextRange *newRange = [TIMYYTextRange rangeWithStart:(isStart ? position : _selectedTextRange.start)
...
- (void)_updateTextRangeByTrackingPreSelect {
    if (!_state.trackingTouch) return;
    TIMYYTextRange *newRange = [self _getClosestTokenRangeAtPoint:_trackingPoint];
    _trackingRange = newRange;
}
...
- (void)_showMagnifierCaret {
    ...
    if (_state.showingMagnifierRanged) {
        ...
        [[TIMYYTextEffectWindow sharedWindow] hideMagnifier:_magnifierRanged];
    }
    ...
    if (!_state.showingMagnifierCaret) {
        ...
        [[TIMYYTextEffectWindow sharedWindow] showMagnifier:_magnifierCaret];
    } else {
        [[TIMYYTextEffectWindow sharedWindow] moveMagnifier:_magnifierCaret];
    }
}
...
- (void)_showMagnifierRanged {
    ...
    if (_state.showingMagnifierCaret) {
        ...
        [[TIMYYTextEffectWindow sharedWindow] hideMagnifier:_magnifierCaret];
    }
    ...
    if (lineIndex < _innerLayout.lines.count) {
        TIMYYTextLine *line = _innerLayout.lines[lineIndex];
        ...
        if (!_state.showingMagnifierRanged) {
            ...
            [[TIMYYTextEffectWindow sharedWindow] showMagnifier:_magnifierRanged];
        } else {
            [[TIMYYTextEffectWindow sharedWindow] moveMagnifier:_magnifierRanged];
        }
    }
}
...
- (void)_showMenu {
    ...
    if (_selectionView.caretVisible) {
        ...
        TIMYYTextSelectionRect *sRect = _selectionView.selectionRects.firstObject;
        rect = sRect.rect;
        ...
        TIMYYTextKeyboardManager *mgr = [TIMYYTextKeyboardManager defaultManager];
...
- (void)_showHighlightAnimated:(BOOL)animated {
    ...
    if (!_highlightLayout) {
        ...
        _highlightLayout = [TIMYYTextLayout layoutWithContainer:_innerContainer text:hiText];
...
- (void)_scrollRangeToVisible:(TIMYYTextRange *)range {
    if (!range) return;
...
- (void)_keyboardChanged {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if ([TIMYYTextKeyboardManager defaultManager].keyboardVisible) {
            [self _scrollRangeToVisible:_selectedTextRange];
        } else {
...
- (void)_startLongPressTimer {
    ...
    _longPressTimer = [NSTimer timerWithTimeInterval:kLongPressMinimumDuration
                                              target:[TIMYYWeakProxy proxyWithTarget:self]
...
- (void)_trackDidLongPress {
    ...
    if (_state.showingHighlight) {
        ...
        if (_highlight.longPressAction) {
            ...
            CGRect rect = [_innerLayout rectForRange:[TIMYYTextRange rangeWithRange:_highlightRange]];
            rect = [self _convertRectFromLayout:rect];
        ...
        if (_state.trackingTouch) {
            if (_state.trackingGrabber) {
                ...
                if (newPos) {
                    ...
                    _trackingRange = [TIMYYTextRange rangeWithRange:NSMakeRange(newPos.offset, 0) affinity:newPos.affinity];
                    [self _updateSelectionView];
                }
                [self _hideMenu];
...
- (void)_fixSelectionDot {
    ...
    CGPoint origin = [self convertPoint:CGPointZero toViewOrWindow:[TIMYYTextEffectWindow sharedWindow]];
...
- (TIMYYTextRange *)_getClosestTokenRangeAtPosition:(TIMYYTextPosition *)position {
    position = [self _correctedTextPosition:position];
    if (!position) return nil;
    TIMYYTextRange *range = nil;
    if (_tokenizer) {
    ...
    if (!range || range.asRange.length == 0) {
        ...
        TIMYYTextRange *extStart = [_innerLayout textRangeByExtendingPosition:range.start];
        TIMYYTextRange *extEnd = [_innerLayout textRangeByExtendingPosition:range.end];
        if (extStart && extEnd) {
            NSArray *arr = [@[extStart.start, extStart.end, extEnd.start, extEnd.end] sortedArrayUsingSelector:@selector(compare:)];
            range = [TIMYYTextRange rangeWithStart:arr.firstObject end:arr.lastObject];
        }
    }
    
    range = [self _correctedTextRange:range];
    if (range.asRange.length == 0) {
        range = [TIMYYTextRange rangeWithRange:NSMakeRange(0, _innerText.length)];
    }
...
/// Try to get the character range/position with word granularity from the tokenizer.
- (TIMYYTextRange *)_getClosestTokenRangeAtPoint:(CGPoint)point {
    point = [self _convertPointToLayout:point];
    TIMYYTextRange *touchRange = [_innerLayout closestTextRangeAtPoint:point];
    touchRange = [self _correctedTextRange:touchRange];
    
    if (_tokenizer && touchRange) {
        TIMYYTextRange *encEnd = (id)[_tokenizer rangeEnclosingPosition:touchRange.end withGranularity:UITextGranularityWord inDirection:UITextStorageDirectionBackward];
        TIMYYTextRange *encStart = (id)[_tokenizer rangeEnclosingPosition:touchRange.start withGranularity:UITextGranularityWord inDirection:UITextStorageDirectionForward];
        if (encEnd && encStart) {
            NSArray *arr = [@[encEnd.start, encEnd.end, encStart.start, encStart.end] sortedArrayUsingSelector:@selector(compare:)];
            touchRange = [TIMYYTextRange rangeWithStart:arr.firstObject end:arr.lastObject];
        }
    }
    
    if (touchRange) {
        TIMYYTextRange *extStart = [_innerLayout textRangeByExtendingPosition:touchRange.start];
        TIMYYTextRange *extEnd = [_innerLayout textRangeByExtendingPosition:touchRange.end];
        if (extStart && extEnd) {
            NSArray *arr = [@[extStart.start, extStart.end, extEnd.start, extEnd.end] sortedArrayUsingSelector:@selector(compare:)];
            touchRange = [TIMYYTextRange rangeWithStart:arr.firstObject end:arr.lastObject];
        }
    ...
    if (!touchRange) touchRange = [TIMYYTextRange defaultRange];
    
    if (_innerText.length && touchRange.asRange.length == 0) {
        touchRange = [TIMYYTextRange rangeWithRange:NSMakeRange(0, _innerText.length)];
    }
...
- (TIMYYTextHighlight *)_getHighlightAtPoint:(CGPoint)point range:(NSRangePointer)range {
    if (!_highlightable || !_innerLayout.containsHighlight) return nil;
    point = [self _convertPointToLayout:point];
    TIMYYTextRange *textRange = [_innerLayout textRangeAtPoint:point];
    ...
    TIMYYTextHighlight *highlight = [text attribute:YYTextHighlightAttributeName
                                         atIndex:startIndex
...
- (CGFloat)_getMagnifierRangedOffset {
    ...
    TIMYYTextPosition *position = [_innerLayout closestPositionToPoint:magPoint];
    NSUInteger lineIndex = [_innerLayout lineIndexForPosition:position];
    if (lineIndex < _innerLayout.lines.count) {
        TIMYYTextLine *line = _innerLayout.lines[lineIndex];
        if (_verticalForm) {
...
- (BOOL)_isTextPositionValid:(TIMYYTextPosition *)position {
...
- (BOOL)_isTextRangeValid:(TIMYYTextRange *)range {
...
- (TIMYYTextPosition *)_correctedTextPosition:(TIMYYTextPosition *)position {
    if (!position) return nil;
    if ([self _isTextPositionValid:position]) return position;
    if (position.offset < 0) {
        return [TIMYYTextPosition positionWithOffset:0];
    }
    if (position.offset > _innerText.length) {
        return [TIMYYTextPosition positionWithOffset:_innerText.length];
    }
    if (position.offset == 0 && position.affinity == YYTextAffinityBackward) {
        return [TIMYYTextPosition positionWithOffset:position.offset];
    }
...
- (TIMYYTextRange *)_correctedTextRange:(TIMYYTextRange *)range {
    if (!range) return nil;
    if ([self _isTextRangeValid:range]) return range;
    TIMYYTextPosition *start = [self _correctedTextPosition:range.start];
    TIMYYTextPosition *end = [self _correctedTextPosition:range.end];
    return [TIMYYTextRange rangeWithStart:start end:end];
}
...
- (void)_replaceRange:(TIMYYTextRange *)range withText:(NSString *)text notifyToDelegate:(BOOL)notify{
    if (NSEqualRanges(range.asRange, _selectedTextRange.asRange)) {
        ...
        _selectedTextRange = [TIMYYTextRange rangeWithRange:newRange];
        ...
        if (range.asRange.length != text.length) {
            ...
            if (unionRange.length == 0) {
                ...
                _selectedTextRange = [TIMYYTextRange rangeWithRange:NSMakeRange(range.start.offset + text.length, 0)];
...
- (BOOL)_parseText {
    if (self.textParser) {
        TIMYYTextRange *oldTextRange = _selectedTextRange;
        NSRange newRange = _selectedTextRange.asRange;
        ...
        TIMYYTextRange *newTextRange = [TIMYYTextRange rangeWithRange:newRange];
        ...
        switch (result.resultType) {
            ...
            case NSTextCheckingTypePhoneNumber: {
                ...
                if (_highlightTextAttributes.count) {
                    TIMYYTextHighlight *highlight = [TIMYYTextHighlight highlightWithAttributes:_highlightTextAttributes];
...
- (void)_initTextView {
    ...
    // UITextInput
    _selectedTextRange = [TIMYYTextRange defaultRange];
    ...
    _innerContainer = [TIMYYTextContainer new];
    ...
    TIMYYTextHighlight *highlight = [TIMYYTextHighlight new];
    TIMYYTextBorder * border = [TIMYYTextBorder new];
    ...
    _containerView = [TIMYYTextContainerView new];
    _containerView.hostView = self;
    
    _selectionView = [TIMYYTextSelectionView new];
    _selectionView.userInteractionEnabled = NO;
    _selectionView.hostView = self;
    _selectionView.color = [self _defaultTintColor];
    
    _magnifierCaret = [TIMYYTextMagnifier magnifierWithType:YYTextMagnifierTypeCaret];
    _magnifierCaret.hostView = _containerView;
    _magnifierRanged = [TIMYYTextMagnifier magnifierWithType:YYTextMagnifierTypeRanged];
    ...
    self.debugOption = [TIMYYTextDebugOption sharedDebugOption];
    [TIMYYTextDebugOption addDebugTarget:self];
    ...
    [[TIMYYTextKeyboardManager defaultManager] addObserver:self];
...
- (void)dealloc {
    ...
    [[TIMYYTextKeyboardManager defaultManager] removeObserver:self];
    
    [[TIMYYTextEffectWindow sharedWindow] hideSelectionDot:_selectionView];
    [[TIMYYTextEffectWindow sharedWindow] hideMagnifier:_magnifierCaret];
    [[TIMYYTextEffectWindow sharedWindow] hideMagnifier:_magnifierRanged];
    
    [TIMYYTextDebugOption removeDebugTarget:self];
...
- (void)scrollRangeToVisible:(NSRange)range {
    TIMYYTextRange *textRange = [TIMYYTextRange rangeWithRange:range];
    textRange = [self _correctedTextRange:textRange];
...
- (void)setText:(NSString *)text {
    ...
    [self replaceRange:[TIMYYTextRange rangeWithRange:NSMakeRange(0, _innerText.length)] withText:text];
}
...
- (void)setTextParser:(id<YYTextParser>)textParser {
    ...
    if (textParser && _text.length) {
        [self replaceRange:[TIMYYTextRange rangeWithRange:NSMakeRange(0, _text.length)] withText:_text];
    }
    [self _resetUndoAndRedoStack];
    [self _commitUpdate];
}
...
- (void)setAttributedText:(NSAttributedString *)attributedText {
    ...
    if (text.length == 0) {
        [self replaceRange:[TIMYYTextRange rangeWithRange:NSMakeRange(0, _innerText.length)] withText:@""];
        return;
    }
    ...
    [_inputDelegate textWillChange:self];
     _innerText = text;
    [self _parseText];
    _selectedTextRange = [TIMYYTextRange rangeWithRange:NSMakeRange(0, _innerText.length)];
    [_inputDelegate textDidChange:self];
    [_inputDelegate selectionDidChange:self];
...
- (void)setSelectedRange:(NSRange)selectedRange {
    ...
    TIMYYTextRange *range = [TIMYYTextRange rangeWithRange:selectedRange];
    range = [self _correctedTextRange:range];
    [self _endTouchTracking];
...
- (void)setDebugOption:(TIMYYTextDebugOption *)debugOption {
    _containerView.debugOption = debugOption;
}

- (TIMYYTextDebugOption *)debugOption {
    return _containerView.debugOption;
}

- (TIMYYTextLayout *)textLayout {
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    TIMYYTextLayout *layout = [TIMYYTextLayout layoutWithContainer:container text:_innerText];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_state.trackingTouch) {
        ...
        if (_highlight) {
            ...
            TIMYYTextHighlight *highlight = [self _getHighlightAtPoint:_trackingPoint range:NULL];
            ...
            if (_state.trackingCaret) {
                ...
                if (!_state.touchMoved) {
                    if (_state.selectedWithoutEdit) {
                        ...
                        if (self.isFirstResponder) {
                            TIMYYTextRange *_oldRange = _trackingRange;
                            [self _updateTextRangeByTrackingCaret];
...
- (void)paste:(id)sender {
    ...
    if (!atr && _allowsPasteImage) {
        UIImage *img = nil;
        if (p.GIFData) {
            img = [TIMYYImage imageWithData:p.GIFData scale:kScreenScale];
        }
        if (!img && p.PNGData) {
            img = [TIMYYImage imageWithData:p.PNGData scale:kScreenScale];
        }
        if (!img && p.WEBPData) {
            img = [TIMYYImage imageWithData:p.WEBPData scale:kScreenScale];
        }
        if (!img) {
            img = p.image;
        }
    ...
    if (atr) {
        ...
        TIMYYTextPosition *pos = [self _correctedTextPosition:[TIMYYTextPosition positionWithOffset:endPosition]];
        TIMYYTextRange *range = [_innerLayout textRangeByExtendingPosition:pos];
        range = [self _correctedTextRange:range];
        if (range) {
...
- (void)select:(id)sender {
    ...
    TIMYYTextRange *newRange = [self _getClosestTokenRangeAtPosition:_selectedTextRange.start];
...
- (void)deleteBackward {
    ...
    if (!_state.deleteConfirm && range.length == 0 && range.location > 0) {
        ...
        TIMYYTextBinding *binding = [_innerText attribute:YYTextBindingAttributeName atIndex:range.location - 1 longestEffectiveRange:&effectiveRange inRange:NSMakeRange(0, _innerText.length)];
        if (binding && binding.deleteConfirm) {
            _state.deleteConfirm = YES;
            [_inputDelegate selectionWillChange:self];
            _selectedTextRange = [TIMYYTextRange rangeWithRange:effectiveRange];
            _selectedTextRange = [self _correctedTextRange:_selectedTextRange];
            [_inputDelegate selectionDidChange:self];
    ...
    _state.deleteConfirm = NO;
    if (range.length == 0) {
        TIMYYTextRange *extendRange = [_innerLayout textRangeByExtendingPosition:_selectedTextRange.end inDirection:UITextLayoutDirectionLeft offset:1];
    ...
    [self replaceRange:[TIMYYTextRange rangeWithRange:range] withText:@""];
}
...
- (void)setSelectedTextRange:(TIMYYTextRange *)selectedTextRange {
...
- (void)setMarkedText:(NSString *)markedText selectedRange:(NSRange)selectedRange {
    ...
    if (_markedTextRange == nil) {
        _markedTextRange = [TIMYYTextRange rangeWithRange:NSMakeRange(_selectedTextRange.end.offset, markedText.length)];
        [_innerText replaceCharactersInRange:NSMakeRange(_selectedTextRange.end.offset, 0) withString:markedText];
        _selectedTextRange = [TIMYYTextRange rangeWithRange:NSMakeRange(_selectedTextRange.start.offset + selectedRange.location, selectedRange.length)];
    } else {
        ...
        _markedTextRange = [TIMYYTextRange rangeWithRange:NSMakeRange(_markedTextRange.start.offset, markedText.length)];
        _selectedTextRange = [TIMYYTextRange rangeWithRange:NSMakeRange(_markedTextRange.start.offset + selectedRange.location, selectedRange.length)];
    }
...
- (void)replaceRange:(TIMYYTextRange *)range withText:(NSString *)text {
    if (!range) return;
    if (!text) text = @"";
...
- (void)setBaseWritingDirection:(UITextWritingDirection)writingDirection forRange:(TIMYYTextRange *)range {
    if (!range) return;
    range = [self _correctedTextRange:range];
...
- (NSString *)textInRange:(TIMYYTextRange *)range {
    range = [self _correctedTextRange:range];
    if (!range) return @"";
    return [_innerText.string substringWithRange:range.asRange];
}

- (UITextWritingDirection)baseWritingDirectionForPosition:(TIMYYTextPosition *)position inDirection:(UITextStorageDirection)direction {
    [self _updateIfNeeded];
    position = [self _correctedTextPosition:position];
...
- (TIMYYTextPosition *)beginningOfDocument {
    return [TIMYYTextPosition positionWithOffset:0];
}

- (TIMYYTextPosition *)endOfDocument {
    return [TIMYYTextPosition positionWithOffset:_innerText.length];
}

- (TIMYYTextPosition *)positionFromPosition:(TIMYYTextPosition *)position offset:(NSInteger)offset {
    ...
    if (newLocation != 0 && newLocation != _innerText.length) {
        // fix emoji
        [self _updateIfNeeded];
        TIMYYTextRange *extendRange = [_innerLayout textRangeByExtendingPosition:[TIMYYTextPosition positionWithOffset:newLocation]];
    ...
    TIMYYTextPosition *p = [TIMYYTextPosition positionWithOffset:newLocation];
    return [self _correctedTextPosition:p];
}

- (TIMYYTextPosition *)positionFromPosition:(TIMYYTextPosition *)position inDirection:(UITextLayoutDirection)direction offset:(NSInteger)offset {
    [self _updateIfNeeded];
    TIMYYTextRange *range = [_innerLayout textRangeByExtendingPosition:position inDirection:direction offset:offset];
    ...
    TIMYYTextPosition *newPosition = forward ? range.end : range.start;
    if (newPosition.offset > _innerText.length) {
        newPosition = [TIMYYTextPosition positionWithOffset:_innerText.length affinity:YYTextAffinityBackward];
    }
    
    return [self _correctedTextPosition:newPosition];
}

- (TIMYYTextRange *)textRangeFromPosition:(TIMYYTextPosition *)fromPosition toPosition:(TIMYYTextPosition *)toPosition {
    return [TIMYYTextRange rangeWithStart:fromPosition end:toPosition];
}

- (NSComparisonResult)comparePosition:(TIMYYTextPosition *)position toPosition:(TIMYYTextPosition *)other {
    return [position compare:other];
}

- (NSInteger)offsetFromPosition:(TIMYYTextPosition *)from toPosition:(TIMYYTextPosition *)toPosition {
    return toPosition.offset - from.offset;
}

- (TIMYYTextPosition *)positionWithinRange:(TIMYYTextRange *)range farthestInDirection:(UITextLayoutDirection)direction {
    NSRange nsRange = range.asRange;
    if (direction == UITextLayoutDirectionLeft | direction == UITextLayoutDirectionUp) {
        return [TIMYYTextPosition positionWithOffset:nsRange.location];
    } else {
        return [TIMYYTextPosition positionWithOffset:nsRange.location + nsRange.length affinity:YYTextAffinityBackward];
    }
}

- (TIMYYTextRange *)characterRangeByExtendingPosition:(TIMYYTextPosition *)position inDirection:(UITextLayoutDirection)direction {
    [self _updateIfNeeded];
    TIMYYTextRange *range = [_innerLayout textRangeByExtendingPosition:position inDirection:direction offset:1];
    return [self _correctedTextRange:range];
}

- (TIMYYTextPosition *)closestPositionToPoint:(CGPoint)point {
    [self _updateIfNeeded];
    point = [self _convertPointToLayout:point];
    TIMYYTextPosition *position = [_innerLayout closestPositionToPoint:point];
    return [self _correctedTextPosition:position];
}

- (TIMYYTextPosition *)closestPositionToPoint:(CGPoint)point withinRange:(TIMYYTextRange *)range {
    TIMYYTextPosition *pos = (id)[self closestPositionToPoint:point];
    if (!pos) return nil;
...
- (TIMYYTextRange *)characterRangeAtPoint:(CGPoint)point {
    [self _updateIfNeeded];
    point = [self _convertPointToLayout:point];
    TIMYYTextRange *r = [_innerLayout closestTextRangeAtPoint:point];
    return [self _correctedTextRange:r];
}

- (CGRect)firstRectForRange:(TIMYYTextRange *)range {
    [self _updateIfNeeded];
...
- (CGRect)caretRectForPosition:(TIMYYTextPosition *)position {
    [self _updateIfNeeded];
...
- (NSArray *)selectionRectsForRange:(TIMYYTextRange *)range {
    [self _updateIfNeeded];
    NSArray *rects = [_innerLayout selectionRectsForRange:range];
    [rects enumerateObjectsUsingBlock:^(TIMYYTextSelectionRect *rect, NSUInteger idx, BOOL *stop) {
        rect.rect = [self _convertRectFromLayout:rect.rect];
    }];
    return rects;
}
...
- (void)setSelectionAffinity:(UITextStorageDirection)selectionAffinity {
    _selectedTextRange = [TIMYYTextRange rangeWithRange:_selectedTextRange.asRange affinity:selectionAffinity == UITextStorageDirectionForward ? YYTextAffinityForward : YYTextAffinityBackward];
    [self _updateSelectionView];
}

- (NSDictionary *)textStylingAtPosition:(TIMYYTextPosition *)position inDirection:(UITextStorageDirection)direction {
    if (!position) return nil;
...
- (TIMYYTextPosition *)positionWithinRange:(TIMYYTextRange *)range atCharacterOffset:(NSInteger)offset {
    if (!range) return nil;
    ...
    else return [TIMYYTextPosition positionWithOffset:offset];
}

- (NSInteger)characterOffsetOfPosition:(TIMYYTextPosition *)position withinRange:(TIMYYTextRange *)range {
...
@interface TIMYYTextView(IBInspectableProperties)
@end

@implementation TIMYYTextView(IBInspectableProperties)
...
- (void)setDebugEnabled_:(BOOL)enabled {
    if (!enabled) {
        ...
        TIMYYTextDebugOption *debugOption = [TIMYYTextDebugOption new];
