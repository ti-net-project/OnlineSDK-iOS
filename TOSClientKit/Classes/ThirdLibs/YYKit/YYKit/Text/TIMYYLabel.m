...
@interface TIMYYLabel() <TIMYYTextDebugTarget, TIMYYAsyncLayerDelegate> {
    ...
    TIMYYTextLayout *_shrinkInnerLayout;
    TIMYYTextLayout *_shrinkHighlightLayout;
...
- (void)_updateLayout {
    _innerLayout = [TIMYYTextLayout layoutWithContainer:_innerContainer text:_innerText];
    _shrinkInnerLayout = [TIMYYLabel _shrinkLayoutWithLayout:_innerLayout];
}
...
- (void)_clearInnerLayout {
    ...
    TIMYYTextLayout *layout = _innerLayout;
    _innerLayout = nil;
...
- (TIMYYTextLayout *)_highlightLayout {
    return _shrinkHighlightLayout ? _shrinkHighlightLayout : _highlightLayout;
}

+ (TIMYYTextLayout *)_shrinkLayoutWithLayout:(TIMYYTextLayout *)layout {
    if (layout.text.length && layout.lines.count == 0) {
        TIMYYTextContainer *container = layout.container.copy;
        ...
        return [TIMYYTextLayout layoutWithContainer:container text:layout.text];
    } else {
        return nil;
    }
}
...
- (void)_trackDidLongPress {
    ...
    if (_state.hasLongPressAction && _textLongPressAction) {
        ...
        TIMYYTextRange *textRange = [self._innerLayout textRangeAtPoint:point];
    ...
    if (_highlight) {
        ...
        if (longPressAction) {
            TIMYYTextPosition *start = [TIMYYTextPosition positionWithOffset:_highlightRange.location];
            TIMYYTextPosition *end = [TIMYYTextPosition positionWithOffset:_highlightRange.location + _highlightRange.length affinity:YYTextAffinityBackward];
            TIMYYTextRange *range = [TIMYYTextRange rangeWithStart:start end:end];
...
- (TIMYYTextHighlight *)_getHighlightAtPoint:(CGPoint)point range:(NSRangePointer)range {
    ...
    TIMYYTextHighlight *highlight = [_innerText attribute:YYTextHighlightAttributeName
                                               atIndex:startIndex
...
- (void)_showHighlightAnimated:(BOOL)animated {
    ...
    if (!_highlightLayout) {
        ...
        _shrinkHighlightLayout = [TIMYYLabel _shrinkLayoutWithLayout:_highlightLayout];
...
- (void)_initLabel {
    ((TIMYYAsyncLayer *)self.layer).displaysAsynchronously = NO;
    ...
    _debugOption = [TIMYYTextDebugOption sharedDebugOption];
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    if ((!_verticalForm && size.width == self.bounds.size.width) ||
        ...
        TIMYYTextLayout *layout = self._innerLayout;
        BOOL contains = NO;
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    [self _initLabel];
    TIMYYTextContainer *innerContainer = [aDecoder decodeObjectForKey:@"innerContainer"];
    if (innerContainer) {
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_state.trackingTouch) {
        ...
        if (_state.touchMoved && _highlight) {
            TIMYYTextHighlight *highlight = [self _getHighlightAtPoint:point range:NULL];
...
- (void)setTextLayout:(TIMYYTextLayout *)textLayout {
...
- (void)setDisplaysAsynchronously:(BOOL)displaysAsynchronously {
    ...
    ((TIMYYAsyncLayer *)self.layer).displaysAsynchronously = displaysAsynchronously;
}
...
#pragma mark - TIMYYTextDebugTarget
...
- (TIMYYAsyncLayerDisplayTask *)newAsyncDisplayTask {
    ...
    TIMYYTextContainer *container = _innerContainer;
    YYTextVerticalAlignment verticalAlignment = _textVerticalAlignment;
    TIMYYTextDebugOption *debug = _debugOption;
    ...
    __block TIMYYTextLayout *layout = (_state.showingHighlight && _highlightLayout) ? self._highlightLayout : self._innerLayout;
    __block TIMYYTextLayout *shrinkLayout = nil;
    ...
    // create display task
    TIMYYAsyncLayerDisplayTask *task = [TIMYYAsyncLayerDisplayTask new];
    ...
    task.display = ^(CGContextRef context, CGSize size, BOOL (^isCancelled)(void)) {
        ...
        TIMYYTextLayout *drawLayout = layout;
        if (layoutNeedUpdate) {
            layout = [TIMYYTextLayout layoutWithContainer:container text:text];
            shrinkLayout = [TIMYYLabel _shrinkLayoutWithLayout:layout];
            if (isCancelled()) return;
            layoutUpdated = YES;
    ...
    task.didDisplay = ^(CALayer *layer, BOOL finished) {
        ...
        if (!finished) {
            ...
            for (TIMYYTextAttachment *a in drawLayout.attachments) {
        ...
        __strong TIMYYLabel *view = (TIMYYLabel *)layer.delegate;
        if (!view) return;
...
@interface TIMYYLabel(IBInspectableProperties)
@end

@implementation TIMYYLabel (IBInspectableProperties)
