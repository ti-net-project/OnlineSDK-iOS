...
@implementation TIMYYTextArchiver

+ (NSData *)archivedDataWithRootObject:(id)rootObject {
    if (!rootObject) return nil;
    NSMutableData *data = [NSMutableData data];
    TIMYYTextArchiver *archiver = [[[self class] alloc] initForWritingWithMutableData:data];
...
- (id)archiver:(NSKeyedArchiver *)archiver willEncodeObject:(id)object {
    ...
    if (typeID == CTRunDelegateTypeID()) {
        ...
        TIMYYTextRubyAnnotation *ruby = [TIMYYTextRubyAnnotation rubyWithCTRubyRef:ctRuby];
        if (ruby) return ruby;
...
@implementation TIMYYTextUnarchiver

+ (id)unarchiveObjectWithData:(NSData *)data {
    if (data.length == 0) return nil;
    TIMYYTextUnarchiver *unarchiver = [[self alloc] initForReadingWithData:data];
...
- (id)unarchiver:(NSKeyedUnarchiver *)unarchiver didDecodeObject:(id) NS_RELEASES_ARGUMENT object NS_RETURNS_RETAINED {
    if ([object class] == [TIMYYTextRunDelegate class]) {
        TIMYYTextRunDelegate *runDelegate = object;
        CTRunDelegateRef ct = runDelegate.CTRunDelegate;
        id ctObj = (__bridge id)ct;
        if (ct) CFRelease(ct);
        return ctObj;
    } else if ([object class] == [TIMYYTextRubyAnnotation class]) {
        TIMYYTextRubyAnnotation *ruby = object;
        if (kiOS8Later) {
