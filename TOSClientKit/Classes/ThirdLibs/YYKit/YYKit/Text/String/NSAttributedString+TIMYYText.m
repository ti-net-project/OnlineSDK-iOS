...
@implementation NSAttributedString (TIMYYText)

- (NSData *)archiveToData {
    NSData *data = nil;
    @try {
        data = [TIMYYTextArchiver archivedDataWithRootObject:self];
    }
...
+ (instancetype)unarchiveFromData:(NSData *)data {
    ...
    @try {
        one = [TIMYYTextUnarchiver unarchiveObjectWithData:data];
    }
    @catch (NSException *exception) {
        NSLog(@"%@",exception);
    }
...
- (NSString *)plainTextForRange:(NSRange)range {
    ...
    [self enumerateAttribute:YYTextBackedStringAttributeName inRange:range options:kNilOptions usingBlock:^(id value, NSRange range, BOOL *stop) {
        TIMYYTextBackedString *backed = value;
...
+ (NSMutableAttributedString *)attachmentStringWithContent:(id)content
    ...
    TIMYYTextAttachment *attach = [TIMYYTextAttachment new];
    attach.content = content;
    attach.contentMode = contentMode;
    [atr setTextAttachment:attach range:NSMakeRange(0, atr.length)];
    
    TIMYYTextRunDelegate *delegate = [TIMYYTextRunDelegate new];
...
+ (NSMutableAttributedString *)attachmentStringWithEmojiImage:(UIImage *)image
    ...
    TIMYYTextAttachment *attachment = [TIMYYTextAttachment new];
    ...
    if (hasAnim) {
        TIMYYAnimatedImageView *view = [TIMYYAnimatedImageView new];
        view.frame = bounding;
        view.image = image;
...
- (void)setTextBackedString:(TIMYYTextBackedString *)textBackedString range:(NSRange)range {
    [self setAttribute:YYTextBackedStringAttributeName value:textBackedString range:range];
}

- (void)setTextBinding:(TIMYYTextBinding *)textBinding range:(NSRange)range {
    [self setAttribute:YYTextBindingAttributeName value:textBinding range:range];
}

- (void)setTextShadow:(TIMYYTextShadow *)textShadow range:(NSRange)range {
    [self setAttribute:YYTextShadowAttributeName value:textShadow range:range];
}

- (void)setTextInnerShadow:(TIMYYTextShadow *)textInnerShadow range:(NSRange)range {
    [self setAttribute:YYTextInnerShadowAttributeName value:textInnerShadow range:range];
}

- (void)setTextUnderline:(TIMYYTextDecoration *)textUnderline range:(NSRange)range {
    [self setAttribute:YYTextUnderlineAttributeName value:textUnderline range:range];
}

- (void)setTextStrikethrough:(TIMYYTextDecoration *)textStrikethrough range:(NSRange)range {
    [self setAttribute:YYTextStrikethroughAttributeName value:textStrikethrough range:range];
}

- (void)setTextBorder:(TIMYYTextBorder *)textBorder range:(NSRange)range {
    [self setAttribute:YYTextBorderAttributeName value:textBorder range:range];
}

- (void)setTextBackgroundBorder:(TIMYYTextBorder *)textBackgroundBorder range:(NSRange)range {
    [self setAttribute:YYTextBackgroundBorderAttributeName value:textBackgroundBorder range:range];
}

- (void)setTextAttachment:(TIMYYTextAttachment *)textAttachment range:(NSRange)range {
    [self setAttribute:YYTextAttachmentAttributeName value:textAttachment range:range];
}

- (void)setTextHighlight:(TIMYYTextHighlight *)textHighlight range:(NSRange)range {
    [self setAttribute:YYTextHighlightAttributeName value:textHighlight range:range];
}

- (void)setTextBlockBorder:(TIMYYTextBorder *)textBlockBorder range:(NSRange)range {
    [self setAttribute:YYTextBlockBorderAttributeName value:textBlockBorder range:range];
}

- (void)setTextRubyAnnotation:(TIMYYTextRubyAnnotation *)ruby range:(NSRange)range {
    if (kiOS8Later) {
...
- (void)setTextHighlightRange:(NSRange)range
    ...
    TIMYYTextHighlight *highlight = [TIMYYTextHighlight highlightWithBackgroundColor:backgroundColor];
