...
@implementation TIMYYTextBackedString

+ (instancetype)stringWithString:(NSString *)string {
    TIMYYTextBackedString *one = [self new];
    one.string = string;
    return one;
}
...
@implementation TIMYYTextBinding

+ (instancetype)bindingWithDeleteConfirm:(BOOL)deleteConfirm {
    TIMYYTextBinding *one = [self new];
...
+ (instancetype)shadowWithNSShadow:(NSShadow *)nsShadow {
    ...
    TIMYYTextShadow *shadow = [self new];
...
@implementation TIMYYTextDecoration
...
+ (instancetype)decorationWithStyle:(YYTextLineStyle)style {
    TIMYYTextDecoration *one = [self new];
    one.style = style;
    return one;
}
...
@implementation TIMYYTextAttachment

+ (instancetype)attachmentWithContent:(id)content {
    TIMYYTextAttachment *one = [self new];
    one.content = content;
    return one;
}
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    NSData *data = nil;
    @try {
        data = [TIMYYTextArchiver archivedDataWithRootObject:self.attributes];
    }
...
- (id)initWithCoder:(NSCoder *)aDecoder {
    ...
    @try {
        _attributes = [TIMYYTextUnarchiver unarchiveObjectWithData:data];
    }
    @catch (NSException *exception) {
        NSLog(@"%@",exception);
    }
