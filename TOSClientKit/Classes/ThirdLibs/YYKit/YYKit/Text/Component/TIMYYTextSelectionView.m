...
@implementation TIMYYSelectionGrabberDot
...
@implementation TIMYYSelectionGrabber

- (instancetype) initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    if (!self) return nil;
    _dot = [[TIMYYSelectionGrabberDot alloc] initWithFrame:CGRectMake(0, 0, 10, 10)];
    return self;
}
...
@interface TIMYYTextSelectionView ()
@property (nonatomic, strong) NSTimer *caretTimer;
@property (nonatomic, strong) UIView *caretView;
@property (nonatomic, strong) TIMYYSelectionGrabber *startGrabber;
@property (nonatomic, strong) TIMYYSelectionGrabber *endGrabber;
@property (nonatomic, strong) NSMutableArray *markViews;
@end

@implementation TIMYYTextSelectionView
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    _startGrabber = [TIMYYSelectionGrabber new];
    _startGrabber.dotDirection = YYTextDirectionTop;
    _startGrabber.hidden = YES;
    _endGrabber = [TIMYYSelectionGrabber new];
...
- (void)_startBlinks {
    [_caretTimer invalidate];
    if (_caretVisible) {
        _caretTimer = [NSTimer timerWithTimeInterval:kBlinkDuration target:[TIMYYWeakProxy proxyWithTarget:self] selector:@selector(_doBlink) userInfo:nil repeats:YES];
...
- (void)setSelectionRects:(NSArray *)selectionRects {
    ...
    [selectionRects enumerateObjectsUsingBlock: ^(TIMYYTextSelectionRect *r, NSUInteger idx, BOOL *stop) {
        CGRect rect = r.rect;
...
- (BOOL)isSelectionRectsContainsPoint:(CGPoint)point {
    ...
    for (TIMYYTextSelectionRect *rect in _selectionRects) {
