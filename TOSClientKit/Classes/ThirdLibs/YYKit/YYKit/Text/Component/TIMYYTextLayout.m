...
@implementation TIMYYTextLinePositionSimpleModifier
- (void)modifyLines:(NSArray *)lines fromText:(NSAttributedString *)text inContainer:(TIMYYTextContainer *)container {
    if (container.verticalForm) {
        for (NSUInteger i = 0, max = lines.count; i < max; i++) {
            TIMYYTextLine *line = lines[i];
...
- (id)copyWithZone:(NSZone *)zone {
    TIMYYTextLinePositionSimpleModifier *one = [self.class new];
...
@implementation TIMYYTextContainer {
    @package
...
+ (instancetype)containerWithSize:(CGSize)size insets:(UIEdgeInsets)insets {
    TIMYYTextContainer *one = [self new];
...
- (id)copyWithZone:(NSZone *)zone {
    TIMYYTextContainer *one = [self.class new];
...
@interface TIMYYTextLayout ()

@property (nonatomic, readwrite) TIMYYTextContainer *container;
...
@property (nonatomic, readwrite) TIMYYTextLine *truncatedLine;
...
@implementation TIMYYTextLayout
...
+ (TIMYYTextLayout *)layoutWithContainerSize:(CGSize)size text:(NSAttributedString *)text {
    TIMYYTextContainer *container = [TIMYYTextContainer containerWithSize:size];
    return [self layoutWithContainer:container text:text];
}

+ (TIMYYTextLayout *)layoutWithContainer:(TIMYYTextContainer *)container text:(NSAttributedString *)text {
    return [self layoutWithContainer:container text:text range:NSMakeRange(0, text.length)];
}

+ (TIMYYTextLayout *)layoutWithContainer:(TIMYYTextContainer *)container text:(NSAttributedString *)text range:(NSRange)range {
    TIMYYTextLayout *layout = NULL;
    ...
    TIMYYTextLine *truncatedLine = nil;
    ...
    layout = [[TIMYYTextLayout alloc] _init];
    layout.text = text;
    ...
    for (NSUInteger i = 0; i < lineCount; i++) {
        ...
        TIMYYTextLine *line = [TIMYYTextLine lineWithCTLine:ctLine position:position vertical:isVerticalForm];
    ...
    if (rowCount > 0) {
        if (maximumNumberOfRows > 0) {
            if (rowCount > maximumNumberOfRows) {
                ...
                do {
                    TIMYYTextLine *line = lines.lastObject;
                    if (!line) break;
        ...
        TIMYYTextLine *lastLine = lines.lastObject;
        ...
        if (container.truncationType != YYTextTruncationTypeNone) {
            ...
            if (truncationTokenLine) {
                ...
                if (ctLastLineExtend) {
                    ...
                    if (ctTruncatedLine) {
                        truncatedLine = [TIMYYTextLine lineWithCTLine:ctTruncatedLine position:lastLine.position vertical:isVerticalForm];
    ...
    if (isVerticalForm) {
        ...
        void (^lineBlock)(TIMYYTextLine *) = ^(TIMYYTextLine *line){
            ...
            for (NSUInteger r = 0; r < runCount; r++) {
                ...
                for (NSUInteger g = 0; g < glyphCount; g++) {
                    ...
                    if (g == 0) {
                        ...
                        TIMYYTextRunGlyphRange *aRange = [TIMYYTextRunGlyphRange rangeWithRange:NSMakeRange(prevIdx, g - prevIdx) drawMode:prevMode];
                ...
                if (prevIdx < glyphCount) {
                    TIMYYTextRunGlyphRange *aRange = [TIMYYTextRunGlyphRange rangeWithRange:NSMakeRange(prevIdx, glyphCount - prevIdx) drawMode:prevMode];
                    [runRanges addObject:aRange];
                }
                
            }
        };
        for (TIMYYTextLine *line in lines) {
            lineBlock(line);
        }
    ...
    for (NSUInteger i = 0, max = lines.count; i < max; i++) {
        ...
        if (line.attachments.count > 0) {
            ...
            for (TIMYYTextAttachment *attachment in line.attachments) {
...
+ (NSArray *)layoutWithContainers:(NSArray *)containers text:(NSAttributedString *)text range:(NSRange)range {
    ...
    for (NSUInteger i = 0, max = containers.count; i < max; i++) {
        TIMYYTextContainer *container = containers[i];
        TIMYYTextLayout *layout = [self layoutWithContainer:container text:text range:range];
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    NSData *textData = [TIMYYTextArchiver archivedDataWithRootObject:_text];
...
- (id)initWithCoder:(NSCoder *)aDecoder {
    ...
    NSAttributedString *text = [TIMYYTextUnarchiver unarchiveObjectWithData:textData];
    TIMYYTextContainer *container = [aDecoder decodeObjectForKey:@"container"];
 ...
 */
- (CTRunRef)_runForLine:(TIMYYTextLine *)line position:(TIMYYTextPosition *)position {
...
/**
 ...
 @param block    The block to be executed before returns YES.
            ...
            next:  right position
 */
- (BOOL)_insideComposedCharacterSequences:(TIMYYTextLine *)line position:(NSUInteger)position block:(void (^)(CGFloat left, CGFloat right, NSUInteger prev, NSUInteger next))block {
 ...
 */
- (BOOL)_insideEmoji:(TIMYYTextLine *)line position:(NSUInteger)position block:(void (^)(CGFloat left, CGFloat right, NSUInteger prev, NSUInteger next))block {
    if (!line) return NO;
    CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
...
/**
 ...
 @return YES if RTL.
 */
- (BOOL)_isRightToLeftInLine:(TIMYYTextLine *)line atPoint:(CGPoint)point {
    if (!line) return NO;
    // get write direction
    BOOL RTL = NO;
    CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
 ...
 */
- (TIMYYTextRange *)_correctedRangeWithEdge:(TIMYYTextRange *)range {
    NSRange visibleRange = self.visibleRange;
    TIMYYTextPosition *start = range.start;
    TIMYYTextPosition *end = range.end;
    
    if (start.offset == visibleRange.location && start.affinity == YYTextAffinityBackward) {
        start = [TIMYYTextPosition positionWithOffset:start.offset affinity:YYTextAffinityForward];
    }
    
    if (end.offset == visibleRange.location + visibleRange.length && start.affinity == YYTextAffinityForward) {
        end = [TIMYYTextPosition positionWithOffset:end.offset affinity:YYTextAffinityBackward];
    }
    
    if (start != range.start || end != range.end) {
        range = [TIMYYTextRange rangeWithStart:start end:end];
    }
    return range;
}
...
- (NSUInteger)rowIndexForLine:(NSUInteger)line {
    ...
    return ((TIMYYTextLine *)_lines[line]).row;
}
...
- (NSUInteger)lineIndexForPoint:(CGPoint)point {
    ...
    for (NSUInteger i = lineIdx0; i <= lineIdx1; i++) {
        CGRect bounds = ((TIMYYTextLine *)_lines[i]).bounds;
...
- (CGFloat)offsetForTextPosition:(NSUInteger)position lineIndex:(NSUInteger)lineIndex {
    ...
    TIMYYTextLine *line = _lines[lineIndex];
...
- (TIMYYTextPosition *)closestPositionToPoint:(CGPoint)point {
    ...
    if (position <= _visibleRange.location) {
        return [TIMYYTextPosition positionWithOffset:_visibleRange.location affinity:YYTextAffinityForward];
    } else if (position >= _visibleRange.location + _visibleRange.length) {
        return [TIMYYTextPosition positionWithOffset:_visibleRange.location + _visibleRange.length affinity:YYTextAffinityBackward];
    }
    ...
    TIMYYTextBinding *binding = [_text attribute:YYTextBindingAttributeName atIndex:position longestEffectiveRange:&bindingRange inRange:NSMakeRange(0, _text.length)];
    if (binding && bindingRange.length > 0) {
        NSUInteger headLineIdx = [self lineIndexForPosition:[TIMYYTextPosition positionWithOffset:bindingRange.location]];
        NSUInteger tailLineIdx = [self lineIndexForPosition:[TIMYYTextPosition positionWithOffset:bindingRange.location + bindingRange.length affinity:YYTextAffinityBackward]];
    ...
    if (line.range.length == 0) {
        ...
        return [TIMYYTextPosition positionWithOffset:line.range.location affinity:behind ? YYTextAffinityBackward:YYTextAffinityForward];
    }
    ...
    if (line.range.length <= 2) {
        ...
        if (YYTextIsLinebreakString(str)) { // an empty line ("\r", "\n", "\r\n")
            return [TIMYYTextPosition positionWithOffset:line.range.location];
        }
    }
    ...
    if (position == line.range.location) {
        return [TIMYYTextPosition positionWithOffset:position];
    }
    if (position == line.range.location + line.range.length) {
        return [TIMYYTextPosition positionWithOffset:position affinity:YYTextAffinityBackward];
    }
    ...
    return [TIMYYTextPosition positionWithOffset:position affinity:finalAffinity];
}

- (TIMYYTextPosition *)positionForPoint:(CGPoint)point
                         oldPosition:(TIMYYTextPosition *)oldPosition
                       otherPosition:(TIMYYTextPosition *)otherPosition {
    if (!oldPosition || !otherPosition) {
        return oldPosition;
    }
    TIMYYTextPosition *newPos = [self closestPositionToPoint:point];
    ...
    if (_container.isVerticalForm) {
        if ([oldPosition compare:otherPosition] == NSOrderedAscending) { // search backward
            TIMYYTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionUp offset:1];
            if (range) return range.start;
        } else { // search forward
            TIMYYTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionDown offset:1];
            if (range) return range.end;
        }
    } else {
        if ([oldPosition compare:otherPosition] == NSOrderedAscending) { // search backward
            TIMYYTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionLeft offset:1];
            if (range) return range.start;
        } else { // search forward
            TIMYYTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionRight offset:1];
            if (range) return range.end;
        }
    }
...
- (TIMYYTextRange *)textRangeAtPoint:(CGPoint)point {
    ...
    TIMYYTextPosition *pos = [self closestPositionToPoint:point];
    if (!pos) return nil;
    ...
    if (_container.verticalForm) {
        TIMYYTextRange *range = [self textRangeByExtendingPosition:pos inDirection:(rect.origin.y >= point.y && !RTL) ? UITextLayoutDirectionUp:UITextLayoutDirectionDown offset:1];
        return range;
    } else {
        TIMYYTextRange *range = [self textRangeByExtendingPosition:pos inDirection:(rect.origin.x >= point.x && !RTL) ? UITextLayoutDirectionLeft:UITextLayoutDirectionRight offset:1];
        return range;
    }
}

- (TIMYYTextRange *)closestTextRangeAtPoint:(CGPoint)point {
    TIMYYTextPosition *pos = [self closestPositionToPoint:point];
    if (!pos) return nil;
    ...
    TIMYYTextRange *range = [self textRangeByExtendingPosition:pos inDirection:direction offset:1];
    return range;
}

- (TIMYYTextRange *)textRangeByExtendingPosition:(TIMYYTextPosition *)position {
    ...
    if (position.offset == visibleStart) {
        return [TIMYYTextRange rangeWithRange:NSMakeRange(position.offset, 0)];
    } else if (position.offset == visibleEnd) {
        return [TIMYYTextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:YYTextAffinityBackward];
    }
    
    // binding range
    NSRange tRange;
    TIMYYTextBinding *binding = [_text attribute:YYTextBindingAttributeName atIndex:position.offset longestEffectiveRange:&tRange inRange:_visibleRange];
    if (binding && tRange.length > 0 && tRange.location < position.offset) {
        return [TIMYYTextRange rangeWithRange:tRange];
    }
    ...
    if (lineIndex != NSNotFound) {
        ...
        if (emoji || seq) {
            return [TIMYYTextRange rangeWithRange:NSMakeRange(_prev, _next - _prev)];
        }
    }
    ...
    if (position.offset > visibleStart && position.offset < visibleEnd) {
        ...
        if ((c0 == '\r') && position.offset < visibleEnd) {
            ...
            if (c1 == '\n') {
                return [TIMYYTextRange rangeWithStart:[TIMYYTextPosition positionWithOffset:position.offset - 1] end:[TIMYYTextPosition positionWithOffset:position.offset + 1]];
            }
        }
        if (YYTextIsLinebreakChar(c0) && position.affinity == YYTextAffinityBackward) {
            ...
            return [TIMYYTextRange rangeWithStart:[TIMYYTextPosition positionWithOffset:position.offset - len] end:[TIMYYTextPosition positionWithOffset:position.offset]];
        }
    }
    
    return [TIMYYTextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:position.affinity];
}
...
- (TIMYYTextRange *)textRangeByExtendingPosition:(TIMYYTextPosition *)position
    ...
    if (!forwardMove && position.offset == visibleStart) {
        return [TIMYYTextRange rangeWithRange:NSMakeRange(_visibleRange.location, 0)];
    } else if (forwardMove && position.offset == visibleEnd) {
        return [TIMYYTextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:YYTextAffinityBackward];
    }
    
    // extend from position
    TIMYYTextRange *fromRange = [self textRangeByExtendingPosition:position];
    if (!fromRange) return nil;
    TIMYYTextRange *allForward = [TIMYYTextRange rangeWithStart:fromRange.start end:[TIMYYTextPosition positionWithOffset:visibleEnd]];
    TIMYYTextRange *allBackward = [TIMYYTextRange rangeWithStart:[TIMYYTextPosition positionWithOffset:visibleStart] end:fromRange.end];
    ...
    if (verticalMove) { // up/down in text layout
        ...
        TIMYYTextLine *mostLeftLine = nil, *mostRightLine = nil;
        ...
        TIMYYTextLine *insideLine = _lines[insideIndex];
        NSUInteger pos;
        if (isVerticalForm) {
        ...
        if (afinityEdge) {
            if (pos == insideLine.range.location + insideLine.range.length) {
                ...
                extPos = [TIMYYTextPosition positionWithOffset:pos - lineBreakLen];
            } else {
                extPos = [TIMYYTextPosition positionWithOffset:pos];
            }
        } else {
            extPos = [TIMYYTextPosition positionWithOffset:pos];
        }
        TIMYYTextRange *ext = [self textRangeByExtendingPosition:extPos];
        if (!ext) return nil;
        if (forwardMove) {
            return [TIMYYTextRange rangeWithStart:fromRange.start end:ext.end];
        } else {
            return [TIMYYTextRange rangeWithStart:ext.start end:fromRange.end];
        }
        
    } else { // left/right in text layout
        TIMYYTextPosition *toPosition = [TIMYYTextPosition positionWithOffset:position.offset + (forwardMove ? offset : -offset)];
        if (toPosition.offset <= visibleStart) return allBackward;
        else if (toPosition.offset >= visibleEnd) return allForward;
        
        TIMYYTextRange *toRange = [self textRangeByExtendingPosition:toPosition];
        ...
        return [TIMYYTextRange rangeWithRange:NSMakeRange(start, end - start)];
    }
}

- (NSUInteger)lineIndexForPosition:(TIMYYTextPosition *)position {
...
- (CGPoint)linePositionForPosition:(TIMYYTextPosition *)position {
    NSUInteger lineIndex = [self lineIndexForPosition:position];
...
- (CGRect)firstRectForRange:(TIMYYTextRange *)range {
    ...
    TIMYYTextLine *startLine = _lines[startLineIndex];
    TIMYYTextLine *endLine = _lines[endLineIndex];
    NSMutableArray *lines = [NSMutableArray new];
    for (NSUInteger i = startLineIndex; i <= startLineIndex; i++) {
        TIMYYTextLine *line = _lines[i];
...
- (CGRect)rectForRange:(TIMYYTextRange *)range {
    NSArray *rects = [self selectionRectsForRange:range];
    if (rects.count == 0) return CGRectNull;
    CGRect rectUnion = ((TIMYYTextSelectionRect *)rects.firstObject).rect;
    for (NSUInteger i = 1; i < rects.count; i++) {
        TIMYYTextSelectionRect *rect = rects[i];
...
- (NSArray *)selectionRectsForRange:(TIMYYTextRange *)range {
    ...
    TIMYYTextSelectionRect *start = [TIMYYTextSelectionRect new];
    if (isVertical) {
    ...
    TIMYYTextSelectionRect *end = [TIMYYTextSelectionRect new];
    if (isVertical) {
    ...
    if (startLine.row == endLine.row) { // same row
        ...
        TIMYYTextSelectionRect *rect = [TIMYYTextSelectionRect new];
        if (isVertical) {
        ...
        TIMYYTextSelectionRect *topRect = [TIMYYTextSelectionRect new];
        ...
        TIMYYTextSelectionRect *bottomRect = [TIMYYTextSelectionRect new];
...
- (NSArray *)selectionRectsWithoutStartAndEndForRange:(TIMYYTextRange *)range {
...
- (NSArray *)selectionRectsWithOnlyStartAndEndForRange:(TIMYYTextRange *)range {
    NSMutableArray *rects = [self selectionRectsForRange:range].mutableCopy;
    for (NSInteger i = 0, max = rects.count; i < max; i++) {
        TIMYYTextSelectionRect *rect = rects[i];
        if (!rect.containsStart && !rect.containsEnd) {
...
static void YYTextDrawRun(TIMYYTextLine *line, CTRunRef run, CGContextRef context, CGSize size, BOOL isVertical, NSArray *runRanges, CGFloat verticalOffset) {
    ...
    if (!isVertical && !glyphTransformValue) { // draw run
        ...
        CGContextSaveGState(context); {
            ...
            if (isVertical) {
                ...
                for (TIMYYTextRunGlyphRange *oneRange in runRanges) {
...
static void YYTextDrawBorderRects(CGContextRef context, CGSize size, TIMYYTextBorder *border, NSArray *rects, BOOL isVertical) {
    if (rects.count == 0) return;
    
    TIMYYTextShadow *shadow = border.shadow;
    if (shadow.color) {
        CGContextSaveGState(context);
...
static void YYTextDrawText(TIMYYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    CGContextSaveGState(context); {
...
static void YYTextDrawBlockBorder(TIMYYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    ...
    for (NSInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            TIMYYTextBorder *border = attrs[YYTextBlockBorderAttributeName];
            if (!border) continue;
            
            NSUInteger lineStartIndex = line.index;
            while (lineStartIndex > 0) {
                if (((TIMYYTextLine *)lines[lineStartIndex - 1]).row == line.row) lineStartIndex--;
                else break;
            }
            
            CGRect unionRect = CGRectZero;
            NSUInteger lineStartRow = ((TIMYYTextLine *)lines[lineStartIndex]).row;
            NSUInteger lineContinueIndex = lineStartIndex;
            NSUInteger lineContinueRow = lineStartRow;
            do {
                TIMYYTextLine *one = lines[lineContinueIndex];
                ...
                TIMYYTextLine *next = lines[lineContinueIndex + 1];
                if (next.row != lineContinueRow) {
                    TIMYYTextBorder *nextBorder = [layout.text attribute:YYTextBlockBorderAttributeName atIndex:next.range.location];
...
static void YYTextDrawBorder(TIMYYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, YYTextBorderType type, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    ...
    for (NSInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            TIMYYTextBorder *border = attrs[borderKey];
            if (!border) continue;
            ...
            for (NSInteger ll = l; ll < lMax; ll++) {
                ...
                for (NSInteger rr = (ll == l) ? r : 0, rrMax = CFArrayGetCount(iRuns); rr < rrMax; rr++) {
                    ...
                    TIMYYTextBorder *iBorder = iAttrs[borderKey];
...
static void YYTextDrawDecoration(TIMYYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, YYTextDecorationType type, BOOL (^cancel)(void)) {
    NSArray *lines = layout.lines;
    ...
    for (NSUInteger l = 0, lMax = layout.lines.count; l < lMax; l++) {
        ...
        for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            TIMYYTextDecoration *underline = attrs[YYTextUnderlineAttributeName];
            TIMYYTextDecoration *strikethrough = attrs[YYTextStrikethroughAttributeName];
            ...
            if (needDrawUnderline) {
                ...
                TIMYYTextShadow *shadow = underline.shadow;
                while (shadow) {
                    if (!shadow.color) {
...
static void YYTextDrawAttachment(TIMYYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, UIView *targetView, CALayer *targetLayer, BOOL (^cancel)(void)) {
    
    BOOL isVertical = layout.container.verticalForm;
    CGFloat verticalOffset = isVertical ? (size.width - layout.container.size.width) : 0;
    
    for (NSUInteger i = 0, max = layout.attachments.count; i < max; i++) {
        TIMYYTextAttachment *a = layout.attachments[i];
...
static void YYTextDrawShadow(TIMYYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    ...
    CGContextSaveGState(context); {
        ...
        for (NSUInteger l = 0, lMax = layout.lines.count; l < lMax; l++) {
            ...
            for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
                ...
                NSDictionary *attrs = (id)CTRunGetAttributes(run);
                TIMYYTextShadow *shadow = attrs[YYTextShadowAttributeName];
                TIMYYTextShadow *nsShadow = [TIMYYTextShadow shadowWithNSShadow:attrs[NSShadowAttributeName]]; // NSShadow compatible
                if (nsShadow) {
...
static void YYTextDrawInnerShadow(TIMYYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    CGContextTranslateCTM(context, 0, size.height);
    CGContextScaleCTM(context, 1, -1);
    ...
    for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            CGContextSetTextPosition(context, linePosX, linePosY);
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            TIMYYTextShadow *shadow = attrs[YYTextInnerShadowAttributeName];
            while (shadow) {
                if (!shadow.color) {
                    shadow = shadow.subShadow;
                    continue;
...
static void YYTextDrawDebug(TIMYYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, TIMYYTextDebugOption *op) {
...
- (void)drawInContext:(CGContextRef)context
                ...
                point:(CGPoint)point
                 view:(UIView *)view
                layer:(CALayer *)layer
                debug:(TIMYYTextDebugOption *)debug
...
- (void)removeAttachmentFromViewAndLayer {
    YYAssertMainThread();
    for (TIMYYTextAttachment *a in self.attachments) {
