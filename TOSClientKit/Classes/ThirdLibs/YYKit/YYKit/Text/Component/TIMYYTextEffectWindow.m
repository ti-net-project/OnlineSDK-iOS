...
@implementation TIMYYTextEffectWindow

+ (instancetype)sharedWindow {
    static TIMYYTextEffectWindow *one = nil;
    if (one == nil) {
        // iOS 9 compatible
...
- (YYTextDirection)_keyboardDirection {
    CGRect keyboardFrame = [TIMYYTextKeyboardManager defaultManager].keyboardFrame;
    keyboardFrame = [[TIMYYTextKeyboardManager defaultManager] convertRect:keyboardFrame toView:self];
...
- (CGPoint)_correctedCenter:(CGPoint)center forMagnifier:(TIMYYTextMagnifier *)mag rotation:(CGFloat)rotation {
 ...
 */
- (CGFloat)_updateMagnifier:(TIMYYTextMagnifier *)mag {
    UIApplication *app = [UIApplication sharedExtensionApplication];
    if (!app) return 0;
...
- (void)showMagnifier:(TIMYYTextMagnifier *)mag {
    if (!mag) return;
...
- (void)moveMagnifier:(TIMYYTextMagnifier *)mag {
    if (!mag) return;
    [self _updateWindowLevel];
    CGFloat rotation = [self _updateMagnifier:mag];
    CGPoint center = [self convertPoint:mag.hostPopoverCenter fromViewOrWindow:mag.hostView];
...
- (void)hideMagnifier:(TIMYYTextMagnifier *)mag {
    if (!mag) return;
...
- (void)_updateSelectionGrabberDot:(TIMYYSelectionGrabberDot *)dot selection:(TIMYYTextSelectionView *)selection{
...
- (void)showSelectionDot:(TIMYYTextSelectionView *)selection {
    if (!selection) return;
    [self _updateWindowLevel];
...
- (void)hideSelectionDot:(TIMYYTextSelectionView *)selection {
    if (!selection) return;
