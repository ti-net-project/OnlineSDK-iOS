...
- (id)copyWithZone:(NSZone *)zone {
    TIMYYKeychainItem *item = [TIMYYKeychainItem new];
...
@implementation TIMYYKeychain

+ (NSString *)getPasswordForService:(NSString *)serviceName
                            account:(NSString *)account
                              error:(NSError **)error {
    if (!serviceName || !account) {
        if (error) *error = [TIMYYKeychain errorWithCode:errSecParam];
        return nil;
    }
    ...
    TIMYYKeychainItem *result = [self selectOneItem:item error:error];
...
+ (BOOL)setPassword:(NSString *)password
    ...
    item.service = serviceName;
    item.account = account;
    TIMYYKeychainItem *result = [self selectOneItem:item error:NULL];
    if (result) {
...
+ (BOOL)insertItem:(TIMYYKeychainItem *)item error:(NSError **)error {
    ...
    if (status != errSecSuccess) {
        if (error) *error = [TIMYYKeychain errorWithCode:status];
        return NO;
    }
...
+ (BOOL)insertItem:(TIMYYKeychainItem *)item {
    return [self insertItem:item error:NULL];
}

+ (BOOL)updateItem:(TIMYYKeychainItem *)item error:(NSError **)error {
    if (!item.service || !item.account || !item.passwordData) {
        if (error) *error = [TIMYYKeychain errorWithCode:errSecParam];
        return NO;
    }
...
+ (BOOL)updateItem:(TIMYYKeychainItem *)item {
    return [self updateItem:item error:NULL];
}

+ (BOOL)deleteItem:(TIMYYKeychainItem *)item error:(NSError **)error {
...
+ (BOOL)deleteItem:(TIMYYKeychainItem *)item {
    return [self deleteItem:item error:NULL];
}

+ (TIMYYKeychainItem *)selectOneItem:(TIMYYKeychainItem *)item error:(NSError **)error {
    if (!item.service || !item.account) {
        if (error) *error = [TIMYYKeychain errorWithCode:errSecParam];
        return nil;
    }
    ...
    return [[TIMYYKeychainItem alloc] initWithDic:dic];
}

+ (TIMYYKeychainItem *)selectOneItem:(TIMYYKeychainItem *)item {
    return [self selectOneItem:item error:NULL];
}

+ (NSArray *)selectItems:(TIMYYKeychainItem *)item error:(NSError **)error {
    NSMutableDictionary *query = [item dic];
    ...
    if (CFGetTypeID(result) == CFDictionaryGetTypeID()) {
        dic = (__bridge NSDictionary *)(result);
        TIMYYKeychainItem *item = [[TIMYYKeychainItem alloc] initWithDic:dic];
...
+ (NSArray *)selectItems:(TIMYYKeychainItem *)item {
