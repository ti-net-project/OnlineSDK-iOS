...
static void YYReachabilityCallback(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags, void *info) {
    TIMYYReachability *self = ((__bridge TIMYYReachability *)info);
    if (self.notifyBlock) {
...
@implementation TIMYYReachability
...
+ (instancetype)reachabilityForLocalWifi {
    ...
    TIMYYReachability *one = [self reachabilityWithAddress:(const struct sockaddr *)&localWifiAddress];
    one.allowWWAN = NO;
    return one;
}
...
- (void)setNotifyBlock:(void (^)(TIMYYReachability *reachability))notifyBlock {
