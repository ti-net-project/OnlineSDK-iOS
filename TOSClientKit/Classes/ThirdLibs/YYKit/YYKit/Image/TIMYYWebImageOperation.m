...
@interface TIMYYWebImageOperation() <NSURLConnectionDelegate>
...
@property (nonatomic, strong) TIMYYImageDecoder *progressiveDecoder;
...
@implementation TIMYYWebImageOperation
...
- (void)_startRequest:(id)object {
    ...
    @autoreleasepool {
        ...
        [_lock lock];
        if (![self isCancelled]) {
            _connection = [[NSURLConnection alloc] initWithRequest:_request delegate:[TIMYYWeakProxy proxyWithTarget:self]];
            if (![_request.URL isFileURL] && (_options & YYWebImageOptionShowNetworkActivity)) {
        ...
        if (![self isCancelled]) {
            if (_cache) {
                if (image || (_options & YYWebImageOptionRefreshImageCache)) {
                    NSData *data = _data;
                    dispatch_async([TIMYYWebImageOperation _imageQueue], ^{
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    @autoreleasepool {
        ...
        if (!_progressiveDecoder) {
            _progressiveDecoder = [[TIMYYImageDecoder alloc] initWithScale:[UIScreen mainScreen].scale];
        }
        ...
        if (!progressiveBlur) {
            TIMYYImageFrame *frame = [_progressiveDecoder frameAtIndex:0 decodeForDisplay:YES];
            if (frame.image) {
                [_lock lock];
                if (![self isCancelled]) {
    ...
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            ...
            dispatch_async([self.class _imageQueue], ^{
                ...
                if (allowAnimation) {
                    image = [[TIMYYImage alloc] initWithData:self.data scale:[UIScreen mainScreen].scale];
                    if (shouldDecode) image = [image imageByDecoded];
                    if ([((TIMYYImage *)image) animatedImageFrameCount] > 1) {
                        hasAnimation = YES;
                    }
                } else {
                    TIMYYImageDecoder *decoder = [TIMYYImageDecoder decoderWithData:self.data scale:[UIScreen mainScreen].scale];
