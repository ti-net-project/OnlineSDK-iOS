...
@implementation TIMYYImageFrame
+ (instancetype)frameWithImage:(UIImage *)image {
    TIMYYImageFrame *frame = [self new];
    frame.image = image;
    return frame;
}
- (id)copyWithZone:(NSZone *)zone {
    TIMYYImageFrame *frame = [self.class new];
    frame.index = _index;
    frame.width = _width;
...
@interface _YYImageDecoderFrame : TIMYYImageFrame
...
@implementation TIMYYImageDecoder {
...
+ (instancetype)decoderWithData:(NSData *)data scale:(CGFloat)scale {
    if (!data) return nil;
    TIMYYImageDecoder *decoder = [[TIMYYImageDecoder alloc] initWithScale:scale];
...
- (TIMYYImageFrame *)frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    TIMYYImageFrame *result = nil;
    pthread_mutex_lock(&_lock);
...
- (TIMYYImageFrame *)_frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
...
@implementation TIMYYImageEncoder {
...
+ (NSData *)encodeImage:(UIImage *)image type:(YYImageType)type quality:(CGFloat)quality {
    TIMYYImageEncoder *encoder = [[TIMYYImageEncoder alloc] initWithType:type];
    encoder.quality = quality;
    [encoder addImage:image duration:0];
    return [encoder encode];
}

+ (NSData *)encodeImageWithDecoder:(TIMYYImageDecoder *)decoder type:(YYImageType)type quality:(CGFloat)quality {
...
- (NSData *)_imageDataRepresentationForSystem:(BOOL)forSystem {
    NSData *data = nil;
    if ([self isKindOfClass:[TIMYYImage class]]) {
        TIMYYImage *image = (id)self;
