...
@implementation TIMYYImage {
    TIMYYImageDecoder *_decoder;
    NSArray *_preloadedFrames;
    dispatch_semaphore_t _preloadedLock;
    NSUInteger _bytesPerFrame;
}

+ (TIMYYImage *)imageNamed:(NSString *)name {
...
+ (TIMYYImage *)imageWithContentsOfFile:(NSString *)path {
    return [[self alloc] initWithContentsOfFile:path];
}

+ (TIMYYImage *)imageWithData:(NSData *)data {
    return [[self alloc] initWithData:data];
}

+ (TIMYYImage *)imageWithData:(NSData *)data scale:(CGFloat)scale {
    return [[self alloc] initWithData:data scale:scale];
}
...
- (instancetype)initWithData:(NSData *)data scale:(CGFloat)scale {
    ...
    @autoreleasepool {
        TIMYYImageDecoder *decoder = [TIMYYImageDecoder decoderWithData:data scale:scale];
        TIMYYImageFrame *frame = [decoder frameAtIndex:0 decodeForDisplay:YES];
