...
+ (UIViewController *)XG_GetController {
   // UIApplication.shared.delegate?.window!!.rootViewController
    UIViewController *rootViewController = [[self getMainWindow] rootViewController];
    ...
    if (@available(iOS 13.0, *)) {
        for (UIScene *scene in [UIApplication sharedApplication].connectedScenes) {
            if ([scene isKindOfClass:[UIWindowScene class]] && ((UIWindowScene *)scene).activationState == UISceneActivationStateForegroundActive) {
                ...
                // 尝试获取 key window 或第一个窗口
                for (UIWindow *window in windowScene.windows) {
                    if (window.isKeyWindow) {
                        return window;
                    }
                }
                // 如果没有找到 key window，返回第一个 window
                return windowScene.windows.firstObject;
            }
        }
    }
    
    // 如果没有通过 connectedScenes 获取到，再尝试使用旧版的 UIApplication.shared.windows（用于兼容老设备或无场景的情况）
